 ...
 * spreadsheet display.
 */
public class ConvertSheet2HtmlConverter {
	private Workbook wb;
	...
	public void main(String[] args, HashMap<String, String> imageMap, ArrayList<String> captionHideList) {
		if (args.length < 2) {
//			System.err.println("usage: ToHtml inputWorkbook outputHtmlFile");
		...
		try (FileWriter fw = new FileWriter(args[1]); PrintWriter pw = new PrintWriter(fw)) {
			...
			this.printPage(captionHideList);
		} catch (IOException e) {
	...
	public void printPage(ArrayList<String> captionHideList) throws IOException {
		try {
			ensureOut();
			if (completeHTML) {
	...
	public void print(ArrayList<String> captionHideList) {
		printInlineStyle();
		printSheets(captionHideList);
	}
...
//		}
	...
	private void printSheets(ArrayList<String> captionHideList) {
		ensureOut();
		for (int i = 0; i < wb.getNumberOfSheets(); i++) {
			Sheet sheet = wb.getSheetAt(i);
			printSheet(sheet,captionHideList);
			this.out.format("<br/><br/>");
		}
		
	}
	/*dogfoot 캡션숨긴 아이템 캡션명 삭제 shlim 20200625*/
	public void printSheet(Sheet sheet,ArrayList<String> captionHideList) {
		ensureOut();
//		System.out.println(sheet.getSheetName());
        ...
        for (String captionHideValue : captionHideList) {
            ...
            if(sheet.getSheetName().equals(captionHideValue)) {
            	capCheck = true;
            }
        }
        if(capCheck) {
        	this.out.format("<p style=\"width:100%%; font-size: 20px;\"></p>%n");
        }else {
		...
		try {
			while(iterator.hasNext()){
				...
				if(key.equals(sheet.getSheetName())) {
					k = 1;
					/*dogfoot 여기서 안됨 확인해봐야 함 shlim 20200827*/
	...
	private void printSheetContent(Sheet sheet) {
		...
		while (rows.hasNext()) {
			...
			for (int i = firstColumn; i < endColumn; i++) {
				...
				this.out.format("    <td class=%s %s>%s</td>%n", styleName(style), attrs, content);
			}
			this.out.format("  </tr>%n");
		}
		this.out.format("</tbody>%n");
