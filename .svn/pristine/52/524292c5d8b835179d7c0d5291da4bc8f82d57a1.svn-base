(function () {
    ...
    function forceSpreadFocus() {
                ...
                changed: function (e, args) {
                    switch (args.name) {
                        ...
                        case 'color':
                            ...
                            if (!color) {
                                self._cacheStyle['foreColor'](null);
                            } else if (color.name) {
        ...
        FormatDialog.prototype._initRotateTextTab = function () {
            ...
            function setRotation(e) {
                ...
                if (e.target.className !== "right-textShow" && e.target.className !== "right-text" && e.target.parentElement.className !== "right-text") {
                    var target = $(e.target);
        ...
        FormatDialog.prototype._refreshNumberList = function () {
            ...
            for (var name in groupItems) { /* NOSONAR: Forin*/
                ...
                var result = item.replace("1234.10", numberValue); //To support JP number format use replace.
        ...
        FormatDialog.prototype._getAccountingFormat = function (decimalPlace, selectIndex) {
            // Fix SJS-3607, display the accounting format correctly. By William Chang.
            var format = designer.res.accountingFormats[selectIndex];
            if (decimalPlace > 0) {
                var decimalDigits = "";
                for (var i = 0; i < decimalPlace; i++) {
                    decimalDigits += '0';  // cannot use String.repeat(), it don't support IE.
                }
                var questionMark = decimalDigits.replace(/0/g, "?");
                format = format.replace(/0/g, '0.' + decimalDigits).replace(/\?/g, questionMark);
            }
            return format;
        };
        ...
        FormatDialog.prototype._parseFormat = function (format) {
            ...
            var replaceStr = decimalPlace === 0 ? "" : "\\" + format.substring(format.indexOf('.'), format.indexOf('.') + decimalPlace + 1);
            var tempFormat = format.replace(new RegExp(replaceStr, 'g'), "").replace(/\\/g, "");
            if (format.indexOf('\\\\') === -1) { //See SJS-3148 If contains \\ , it can't be built-in number format.
                for (i = 0; i < numberFormats.length; i++) {
                    if (tempFormat === numberFormats[i]) {
                        ...
                        this._element.find('.negative-number-select').get(0).selectedIndex = (i % (numberFormats.length / 2));
                        if (format.indexOf('#,') !== -1) {
                            useSeparator = true;
            ...
            var tempAccountingFormat = decimalPlace === 0 ? format : format.replace(new RegExp(replaceStr, 'g'), "").replace(new RegExp("\\?{" + decimalPlace + "}", 'g'), "?")
            var accountingFormatsLength = accountingFormats.length;
            for (var j = 0; j < accountingFormatsLength; j++) {
                if (tempAccountingFormat === accountingFormats[j]) {
                    this._element.find('.category-select').val('accounting').trigger('change');
                    this._element.find('.accounting-symbol-dropdown').get(0).selectedIndex = j;
                    return;
                }
            }
    ...
    FormatDialog.prototype._setCommandComboBoxValue = function(element, value) {
        ...
        FormatDialog.prototype._updateCellButtonPreview = function (attribute, value) {
            if (!attribute) {
                return;
            }
            var sheet = this._cellbuttonPreviewSpread.getActiveSheet();
