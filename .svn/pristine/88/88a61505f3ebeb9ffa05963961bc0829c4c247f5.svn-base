(function () {
    ...
    function initDesignerCommands(commandManager) {
        ...
        //Table total row drop down list more functions
    ...
    var SetBackColorAction = (function (_super) {
        ...
        SetBackColorAction.prototype.executeImp = function (context, options, isUndo) {
            var self = this;
            if (options.value === void 0) { // SJS-3642, cannot set color to "no color" if row/column have color.
                options.value = null; 
            }
    ...
    var CreateDefaultTableAction = (function (_super) {
        ...
        CreateDefaultTableAction.prototype.executeImp = function (context, options, isUndo) {
            ...
            var range = options.exRange.ranges[0];
            if (range.row === -1) {
                range.row = 0;
            ...
            self.clearMergeInRange(range, sheet);
            sheet.tables.add(self.getNewTableName(), range.row, range.col, range.rowCount, range.colCount);
        };
        ...
        InsertChartAction.prototype.executeImp = function (context, options, isUndo) {
            try {
                ...
                if (category === chartHelper.chartTypeDict["0"].chartGroup) {
                    ...
                    chart = sheet.charts.add(chartName, chartType, chartPosition.x, chartPosition.y, 480, 300, dataFormula, options.dataOrientation);
                } else {
