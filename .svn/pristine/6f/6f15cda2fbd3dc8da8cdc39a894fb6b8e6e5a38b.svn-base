WISE.libs.Dashboard.item.RangeAreaChartGenerator = function() {
	var self = this;
	
	this.type = 'RANGE_AREA_CHART';
		        ...
		        customizeTooltip: function(_pointInfo) {
					...
					if(_pointInfo.rangeValue1Text !== undefined) {
						text = '<b>' + _pointInfo.argumentText + '</b>' + '<br/>' + 
							'<b>' + _pointInfo.seriesName + '</b>: ' + Number.unit(_pointInfo.rangeValue1, labelFormat, labelUnit, labelPrecision, labelSeparator, undefined, labelSuffix, labelSuffixEnabled, labelPrecision) + 
							' - ' + Number.unit(_pointInfo.rangeValue2, labelFormat, labelUnit, labelPrecision, labelSeparator, undefined, labelSuffix, labelSuffixEnabled, labelPrecision);
					}
					// bubble series
					else if (_pointInfo.sizeText !== undefined) {
						text = '<b>' + _pointInfo.argumentText + '</b>' + '<br/>' + 
							'<b>' + _pointInfo.seriesName + '</b>: ' + Number.unit(_pointInfo.value, labelFormat, labelUnit, labelPrecision, labelSeparator, undefined, labelSuffix, labelSuffixEnabled, labelPrecision) + 
							' - ' + Number.unit(_pointInfo.size, labelFormat, labelUnit, labelPrecision, labelSeparator, undefined, labelSuffix, labelSuffixEnabled, labelPrecision);
					}
					// simple series
					else {
						text = '<div>' + _pointInfo.argumentText + '</div>' + 
							'<div><b>' + _pointInfo.seriesName + '</b>: ' + Number.unit(_pointInfo.value, labelFormat, labelUnit, labelPrecision, labelSeparator, undefined, labelSuffix, labelSuffixEnabled, labelPrecision) + '</div>';
					}
...
//		this.RangeAreaChart['DataItems']['Dimension'] = [];
//		this.RangeAreaChart['DataItems']['Measure'] = [];
		
		this.meta['Values'] = this.RangeAreaChart['Values'] = {Value :D.length != 0 ? D:[]};
		this.meta['Rows'] = this.RangeAreaChart['Rows'] = {Row:R.length != 0 ? R:[]};
		this.meta['Columns'] = this.RangeAreaChart['Columns'] = {Column:C.length != 0 ? C:[]};
		...
		for (var i = 1; i <= this.RangeAreaChart.Values.Value.length; i++) {
			var uniqueName = chartElement['SERIES' + i + '_UNI_NM'],
				...
				Value = {
					Value: {
						UniqueName: (typeof uniqueName !== 'undefined' && uniqueName.length > 0) ? uniqueName : self.RangeAreaChart.Values.Value[i - 1].UNI_NM
					},
...
//					}
		...
		this.RangeAreaChart['Panes'] = self.Panes;
		this.RangeAreaChart['SeriesDimensions'] = this.SeriesDimensions;
		this.RangeAreaChart['Arguments'] = this.Arguments;
	...
	this.__getRangeAreaChartData = function(_seriesType) {
...
//					_data
	...
	this.setRangeAreaChart = function(){
		this.fieldManager.init();
		gDashboard.itemGenerateManager.getherFields(this.fieldManager, self.type, self.isAdhocItem);
		this.RangeAreaChart.DataItems = this.fieldManager.DataItems;
		this.RangeAreaChart.Panes = this.fieldManager.Panes;
		this.RangeAreaChart.SeriesDimensions = this.fieldManager.SeriesDimensions;
		this.RangeAreaChart.Arguments = this.fieldManager.Arguments;
		this.RangeAreaChart.HiddenMeasures = this.fieldManager.HiddenMeasures;
		...
		if (!(this.RangeAreaChart.ComponentName)) {
			this.RangeAreaChart.ComponentName = this.ComponentName;
		}
		if (!(this.RangeAreaChart.DataSource)) {
			this.RangeAreaChart.DataSource = this.dataSourceId;
		}else if(this.RangeAreaChart.DataSource != this.dataSourceId){
			this.RangeAreaChart.DataSource = this.dataSourceId;
		}
		if (!(this.RangeAreaChart.Name)) {
			this.RangeAreaChart.Name = this.Name;
		}
		if (!(this.RangeAreaChart.MemoText)) {
			this.RangeAreaChart.MemoText = this.memoText;
		}
		if (this.RangeAreaChart.InteractivityOptions) {
			if (!(this.RangeAreaChart.InteractivityOptions.MasterFilterMode)) {
				this.RangeAreaChart.InteractivityOptions.MasterFilterMode = 'Off';
			}
			if (!(this.RangeAreaChart.InteractivityOptions.TargetDimensions)) {
				this.RangeAreaChart.InteractivityOptions.TargetDimensions = 'Argument';
			}
			if (!(this.RangeAreaChart.InteractivityOptions.IsDrillDownEnabled)) {
				this.RangeAreaChart.InteractivityOptions.IsDrillDownEnabled = false;
			}
			if (!(this.RangeAreaChart.InteractivityOptions.IgnoreMasterFilters)) {
				this.RangeAreaChart.InteractivityOptions.IgnoreMasterFilters = false;
			}
		} else {
			this.RangeAreaChart.InteractivityOptions = {
		...
		if (!(this.RangeAreaChart.IsMasterFilterCrossDataSource)) {
			this.RangeAreaChart.IsMasterFilterCrossDataSource = false;
		}
		if (!(this.RangeAreaChart.FilterString)) {
			this.RangeAreaChart.FilterString = [];
		}else{
			this.RangeAreaChart.FilterString = JSON.parse(JSON.stringify(this.RangeAreaChart.FilterString).replace(/"@null"/gi,null));
		}
		if (!(this.RangeAreaChart.ShowCaption)) {
			this.RangeAreaChart.ShowCaption = true;
		}
		if (!(this.RangeAreaChart.Rotated)) {
			this.RangeAreaChart.Rotated = false;
		}
		if (!(this.RangeAreaChart.AxisX)) {
			this.RangeAreaChart.AxisX = {
				Visible: true,
				Title: '',
				Rotation: 0
			};
		}
		if (!(this.RangeAreaChart.AxisY)) {
			this.RangeAreaChart.AxisY = {
				FormatType: 'Number',
				Unit: 'Ones',
				ShowZero: true,
				Visible: true,
		...
		if (!(this.RangeAreaChart.ChartLegend)) {
			this.RangeAreaChart.ChartLegend = {
				Visible: true,
		...
		if (!(this.RangeAreaChart.SeriesType)) {
			this.RangeAreaChart.SeriesType = 'bar';
		}
		if (!(this.RangeAreaChart.Palette)) {
			this.RangeAreaChart.Palette = userJsonObject.userPalette ? userJsonObject.userPalette : userJsonObject.defaultPalette;
		}
		if (!(this.RangeAreaChart.Animation)) {
			this.RangeAreaChart.Animation = 'easeOutCubic';
		}	
			
		this.meta = this.RangeAreaChart;
	};
	
	this.setRangeAreaChartForOpen = function(){
		if(typeof this.meta == 'undefined'){
			this.setRangeAreaChart();
		}
		else{
			this.RangeAreaChart=this.meta;
		}
		// load CHART_XML
		...
		if(gDashboard.reportType === 'DashAny'){
			...
			if (typeof gDashboard.structure.MapOption.DASHBOARD_XML !== 'undefined') {
				tempCDE =  WISE.util.Object.toArray(gDashboard.structure.MapOption.DASHBOARD_XML.RANGE_AREA_CHART_DATA_ELEMENT);
				tempDE = WISE.util.Object.toArray(gDashboard.structure.MapOption.DASHBOARD_XML.DATA_ELEMENT);
				tempWCDE = gDashboard.structure.MapOption.DASHBOARD_XML.WEB ? WISE.util.Object.toArray(gDashboard.structure.MapOption.DASHBOARD_XML.WEB.RANGE_AREA_CHART_DATA_ELEMENT) : [];
			} else {
				tempCDE = [];
				tempDE = [];
				tempWCDE = [];
		...
		if(this.RangeAreaChart.Panes == undefined) {
			this.RangeAreaChart.Panes = this.meta.Panes;
		}
		else if(this.fieldManager.initialized) {
			var axisY = this.RangeAreaChart.Panes.Pane.AxisY;
			this.RangeAreaChart.Panes = this.fieldManager.Panes;
		    this.RangeAreaChart.Panes.Pane.AxisY = axisY;		
		
			this.RangeAreaChart.DataItems = this.meta.DataItems = this.fieldManager.DataItems;
			this.RangeAreaChart.SeriesDimensions = this.meta.SeriesDimensions = this.fieldManager.SeriesDimensions;
			this.RangeAreaChart.Arguments = this.meta.Arguments = this.fieldManager.Arguments;
			this.RangeAreaChart.HiddenMeasures =  this.meta.HiddenMeasures = this.fieldManager.HiddenMeasures;
			this.measures = this.fieldManager.measures;
			this.dimensions = this.fieldManager.dimensions;
			this.seriesDimensions = this.fieldManager.seriesDimensions;
			/* DOGFOOT ktkang 그냥 오류나는 경우 있어서 수정  20200110 */
		} else if(typeof this.RangeAreaChart.DataItems != 'undefined'){
			// set point label options from CHART_XML
			$.each(WISE.util.Object.toArray(this.RangeAreaChart.DataItems.Measure), function(mIndex, measure) {
				...
				$.each(WISE.util.Object.toArray(self.RangeAreaChart.Panes.Pane.Series.Simple), function(j, simple) {
				...
				$.each(WISE.util.Object.toArray(self.RangeAreaChart.Panes.Pane.Series.Weighted), function(j, weighted) {
			...
			$.each(WISE.util.Object.toArray(this.RangeAreaChart.DataItems.Measure), function(_i, _mea) {
		...
		if(this.RangeAreaChart.HiddenMeasures != undefined) {
			this.RangeAreaChart.HiddenMeasures.Measure = WISE.util.Object.toArray(this.RangeAreaChart.HiddenMeasures.Measure);
			this.RangeAreaChart.HiddenMeasures.Dimension = WISE.util.Object.toArray(this.RangeAreaChart.HiddenMeasures.Dimension);
		}
		...
		if (this.RangeAreaChart.ShowCaption == undefined) {
			this.RangeAreaChart.ShowCaption = true;
		}
		if (!(this.RangeAreaChart.Rotated)) {
			this.RangeAreaChart.Rotated = false;
		...
		if (!(this.RangeAreaChart.AxisX)) {
			...
			if(gDashboard.reportType === 'DashAny' && typeof chartDataElement.AXISX_OPTION !== 'undefined') {
				self.RangeAreaChart.AxisX.Rotation = chartDataElement.AXISX_OPTION.AXISX_ANGLE;
			}
		}else{
			if(gDashboard.reportType === 'DashAny') {
				self.RangeAreaChart.AxisX.Visible = this.RangeAreaChart.AxisX.Visible;
				self.RangeAreaChart.AxisX.Title = this.RangeAreaChart.AxisX.Title;
				self.RangeAreaChart.AxisX.Rotation = this.RangeAreaChart.AxisX.Rotation;
				...
				if(WISE.Constants.editmode != 'viewer'){
					if(gDashboard.structure.chartXml.AXISX_MAJOR_GRID_ENABLED != undefined){
						self.RangeAreaChart.AxisX.Visible = gDashboard.structure.chartXml.AXISX_MAJOR_GRID_ENABLED != undefined ? gDashboard.structure.chartXml.AXISX_MAJOR_GRID_ENABLED : this.RangeAreaChart.AxisX.Visible;
					...
					if(gDashboard.structure.ReportMasterInfo.chartJson != undefined){
						if(gDashboard.structure.ReportMasterInfo.chartJson.CHART_XML.AXISX_MAJOR_GRID_ENABLED != undefined){
							self.RangeAreaChart.AxisX.Visible = gDashboard.structure.ReportMasterInfo.chartJson.CHART_XML.AXISX_MAJOR_GRID_ENABLED != undefined ? gDashboard.structure.ReportMasterInfo.chartJson.CHART_XML.AXISX_MAJOR_GRID_ENABLED : this.RangeAreaChart.AxisX.Visible;
		...
		if(!(this.RangeAreaChart.AxisY)) {
			if (!(this.RangeAreaChart.Panes.Pane.AxisY)) {
				this.RangeAreaChart.AxisY = {
					FormatType: 'Number',
					Unit: 'Ones',
					ShowZero: true,
				...
				if(gDashboard.reportType == 'DashAny'){
					if(typeof chartDataElement.PANE_ELEMENT != 'undefined') {
						self.RangeAreaChart.AxisY = {
							FormatType: typeof self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.FormatType != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.FormatType : 'Number',
							Unit: typeof self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.Unit != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.Unit : 'Ones',
							ShowZero: typeof self.RangeAreaChart.Panes.Pane.AxisY.ShowZero != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.ShowZero : true,
							Visible: typeof self.RangeAreaChart.Panes.Pane.AxisY.Visible != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.Visible : true,
							Title: typeof self.RangeAreaChart.Panes.Pane.AxisY.Title != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.Title : '',
							SuffixEnabled: chartDataElement.PANE_ELEMENT.USE_AXISY_CUSTOM_UNIT_YN === 'Y',
							MeasureFormat: {
								O: typeof chartDataElement.PANE_ELEMENT.AXISY_UNIT_O != 'undefined' ? chartDataElement.PANE_ELEMENT.AXISY_UNIT_O : "",
								K: typeof chartDataElement.PANE_ELEMENT.AXISY_UNIT_K != 'undefined' ? chartDataElement.PANE_ELEMENT.AXISY_UNIT_K : "천",
							...
							Precision: typeof self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.Precision != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.Precision : 0,
							PrecisionOption: typeof self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.PrecisionOption != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.PrecisionOption : '반올림',
							Separator: typeof self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.IncludeGroupSeparator != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.IncludeGroupSeparator : true
						};
					} else {
						self.RangeAreaChart.AxisY = {
							FormatType: typeof self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.FormatType != 'undefined' ? self.RangeAreaChart.Panes.Pane.AxisY.NumericFormat.FormatType : 'Number',
					...
					if(gDashboard.structure.chartXml.AXISY_MAJOR_GRID_ENABLED != undefined){
						self.RangeAreaChart.AxisY.Visible = gDashboard.structure.chartXml.AXISY_MAJOR_GRID_ENABLED != undefined ? gDashboard.structure.chartXml.AXISY_MAJOR_GRID_ENABLED : this.RangeAreaChart.AxisY.Visible;
		...
		if (this.RangeAreaChart.ChartLegend) {
			var chartLegend = {
				Visible: typeof this.RangeAreaChart.ChartLegend.Visible !== 'undefined' ? this.RangeAreaChart.ChartLegend.Visible : true,
				IsInsidePosition: typeof this.RangeAreaChart.ChartLegend.IsInsidePosition !== 'undefined' ? this.RangeAreaChart.ChartLegend.IsInsidePosition : false,
				InsidePosition: typeof this.RangeAreaChart.ChartLegend.InsidePosition !== 'undefined' ? this.RangeAreaChart.ChartLegend.InsidePosition : 'TopRightHorizontal',
				OutsidePosition: typeof this.RangeAreaChart.ChartLegend.OutsidePosition !== 'undefined' ? this.RangeAreaChart.ChartLegend.OutsidePosition : 'TopRightHorizontal'
			};
			this.RangeAreaChart.ChartLegend = chartLegend;
		} else {
			this.RangeAreaChart.ChartLegend = {
				Visible: true,
				IsInsidePosition: false,
		...
		if (!(this.RangeAreaChart.Animation)) {
			if(typeof chartDataElement.ANIMATION !== 'undefined'){
				this.RangeAreaChart.Animation = chartDataElement.ANIMATION;
			}else{
				this.RangeAreaChart.Animation = 'easeOutCubic';
			}
		}
		...
		if (!(this.RangeAreaChart.Palette)) {
			...
			if (typeof dataElement.PALETTE_NM !== 'undefined') {
				self.RangeAreaChart.Palette = dataElement.PALETTE_NM;
			}
			if(typeof this.meta.ColorSheme != 'undefined'){
				var colorList = WISE.util.Object.toArray(this.meta.ColorSheme.ColorSheme);
				this.RangeAreaChart.Palette = [];
				var newPalette = [];
				$.each(colorList,function(_i,_list){
					self.RangeAreaChart.Palette.push(gDashboard.itemColorManager.getHexColor(_list.Color)); 
	...
	this.setRangeAreaChartForViewer = function() {
		this.RangeAreaChart=this.meta;
		...
		if(gDashboard.reportType == 'AdHoc'){
			this.meta = this.RangeAreaChart = {};
			this.meta.ComponentName  = this.RangeAreaChart.ComponentName = this.ComponentName;
			this.meta.DataSource = this.RangeAreaChart.DataSource = this.dataSourceId;
			this.meta.DataItems = this.RangeAreaChart.DataItems = {};
			if(gDashboard.structure.ReportMasterInfo.reportJson != undefined){
				if(gDashboard.structure.ReportMasterInfo.reportJson.TITLE_ELEMENT != undefined){
					this.meta.Name = this.RangeAreaChart.Name = gDashboard.structure.ReportMasterInfo.reportJson.TITLE_ELEMENT.CHART_TITLE == undefined ? "차트" : gDashboard.structure.ReportMasterInfo.reportJson.TITLE_ELEMENT.CHART_TITLE;
				}
				if(gDashboard.structure.ReportMasterInfo.reportJson.WEB != undefined){
					this.meta.MemoText = this.RangeAreaChart.MemoText = this.memoText = gDashboard.structure.ReportMasterInfo.reportJson.WEB.MEMO_ELEMENT.CHART_MEMO == undefined ? "" : gDashboard.structure.ReportMasterInfo.reportJson.WEB.MEMO_ELEMENT.CHART_MEMO
				
				}else{
					this.meta.MemoText = this.RangeAreaChart.MemoText = this.memoText = "";	
				}
			}
			...
			$.each(gridElement,function(_i,_e){
				if(_e.TYPE == 11){
					...
					self.RangeAreaChart.DataItems.Measure.push(dataItem);
					...
					self.RangeAreaChart.DataItems.Dimension.push(dataItem);
					self.dimensions.push(dataItem);
				}
			});
			this.meta.Values = this.RangeAreaChart.Values = {Value:gDashboard.structure.ReportMasterInfo.reportJson.DATA_ELEMENT ? WISE.util.Object.toArray(gDashboard.structure.ReportMasterInfo.reportJson.DATA_ELEMENT.DATA) : []};
			this.meta.Rows = this.RangeAreaChart.Rows = {Row:gDashboard.structure.ReportMasterInfo.reportJson.ROW_ELEMENT ? WISE.util.Object.toArray(gDashboard.structure.ReportMasterInfo.reportJson.ROW_ELEMENT.ROW) : []};
			this.meta.Columns = this.RangeAreaChart.Columns = {Column:gDashboard.structure.ReportMasterInfo.reportJson.COL_ELEMENT ? WISE.util.Object.toArray(gDashboard.structure.ReportMasterInfo.reportJson.COL_ELEMENT.COL) : []};
...
//						}
			...
			this.RangeAreaChart.ComponentName = this.meta.ComponentName;
			this.RangeAreaChart.DataSource = this.meta.dataSourceId != undefined ? this.meta.dataSourceId:this.meta.DataSource;
			this.RangeAreaChart.Name = this.meta.Name;
			// load CHART_XML
			var tempCDE, tempDE, tempWCDE;
			var page = window.location.pathname.split('/');
			if (typeof gDashboard.structure.MapOption.DASHBOARD_XML !== 'undefined') {
...
//		$("#" + this.itemid).empty();
	...
	this.renderChart = function(_options) {		
		...
		if(_functionDo){
			...
			self.setRangeAreaChart();
			gDashboard.itemGenerateManager.addParentItems(self);
			gDashboard.itemGenerateManager.itemCustomize(self,self.RangeAreaChart);
			gDashboard.itemGenerateManager.generateItem(self, self.RangeAreaChart);
		}
		else if(self.fieldManager) { // 레포트 열기
			if (!($.isEmptyObject(self.RangeAreaChart))) {
				this.fieldManager.seriesType = stylizedChartTypes[self.RangeAreaChart.SeriesType];
				this.fieldManager.previousPanes = self.RangeAreaChart.Panes;
				gDashboard.itemGenerateManager.getherFields(this.fieldManager, self.type, self.isAdhocItem);
			}else{
			...
			this.setRangeAreaChartForOpen();
			gDashboard.itemGenerateManager.addParentItems(self);
			gDashboard.itemGenerateManager.itemCustomize(self,self.RangeAreaChart);
			gDashboard.itemGenerateManager.generateItem(self, self.RangeAreaChart);
		}
		else if(self.meta && $.isEmptyObject(self.RangeAreaChart)) {
			this.setRangeAreaChartForViewer();
			gDashboard.itemGenerateManager.itemCustomize(self,self.RangeAreaChart);
			gDashboard.itemGenerateManager.generateItem(self, self.RangeAreaChart);
		...
		var dataSet = this.__getRangeAreaChartData();
...
//		if (queriedData.length === 0 || this.measures.length === 0) {			
		...
		$.each(this.P, function(_i, _pane) {
			...
			if(typeof self.RangeAreaChart.AxisY.TitleRename == 'undefined' || !self.RangeAreaChart.AxisY.TitleRename) {
				if (yCount === 1) {
			...
			if (self.RangeAreaChart.AxisY) {
				if ((typeof self.RangeAreaChart.AxisY.TitleRename == 'undefined' || self.RangeAreaChart.AxisY.TitleRename) && typeof self.RangeAreaChart.AxisY.Title != 'undefined') {
					yName = self.RangeAreaChart.AxisY.Title;
					/* DOGFOOT ktkang 차트 Y축 이름 다시 조회 시 초기화 오류 수정   20200228 */
				} else {
					self.RangeAreaChart.AxisY.Title = yName;
					self.RangeAreaChart.AxisY.TitleRename = false;
				}
				yFormat = self.RangeAreaChart.AxisY.FormatType;
				yUnit = self.RangeAreaChart.AxisY.Unit;
				yShowZero = self.RangeAreaChart.AxisY.ShowZero === undefined? true : self.RangeAreaChart.AxisY.ShowZero;
				yVisible = self.RangeAreaChart.AxisY.Visible;
				yLabelPrecision = self.RangeAreaChart.AxisY.Precision;
				yLabelPrecisionOption = self.RangeAreaChart.AxisY.PrecisionOption;
				ySeparator = self.RangeAreaChart.AxisY.Separator;
				ySuffixEnabled = self.RangeAreaChart.AxisY.SuffixEnabled;
				ySuffix = self.RangeAreaChart.AxisY.MeasureFormat;
			}
...
//                min: 0,
				...
				label: {
					...
					customizeText: function(e){
						return WISE.util.Number.unit(e.value, yFormat, yUnit, self.RangeAreaChart.SeriesType.indexOf("fullstacked") > -1? yLabelPrecision > 0? yLabelPrecision : 1 : yLabelPrecision, ySeparator, undefined, ySuffix, ySuffixEnabled, yLabelPrecisionOption);
					},
					visible: yVisible,
					overlappingBehavior: {
						mode: 'rotate'
...
//                min: 0,
				...
				label: {
					...
					customizeText: function(e){
						var _seriesType = "";
						$.each(self.RangeAreaChart.Panes.Pane.Series.Simple, function(_i, _series){
						...
						//console.log('e :',e);
						return WISE.util.Number.unit(e.value, yFormat, yUnit, _seriesType? _seriesType.indexOf("FullStacked") > -1? yLabelPrecision > 0? yLabelPrecision : 1 : yLabelPrecision : yLabelPrecision, ySeparator, undefined, self.RangeAreaChart.AxisY.MeasureFormat, self.RangeAreaChart.AxisY.SuffixEnabled, yLabelPrecisionOption);
					},
					visible: yVisible,
					overlappingBehavior: {
						mode: 'rotate'
			...
			$.each(_pane['Series'], function(_seriesType, _serieses) {
				...
				switch(_seriesType) {
				case 'Simple':
					seriesCreator = new WISE.libs.Dashboard.item.RangeAreaChart.SimpleSeries();
					seriesCreator.seriesType = (_options && _options.seriesType) ? _options.seriesType : undefined;
					break;
				case 'Range':
					self.includedRangeSeries = true;
					seriesCreator = new WISE.libs.Dashboard.item.RangeAreaChart.RangeSeries();
					break;
				case 'Weighted':
					seriesCreator = new WISE.libs.Dashboard.item.RangeAreaChart.BubbleSeries();
					break;
				}
				...
				if(seriesElement.length>0)
					self.RangeAreaChart['SeriesType'] = seriesElement[0].type;
			...
			$.each(WISE.util.Object.toArray(self.RangeAreaChart.DataItems.Measure), function(mIndex, measure) {
...
//                            var filter = $('#' + self.itemid + '_editFilter').dxFilterBuilder('instance').getFilterExpression();
			...
			case 'singleMasterFilter': {
				...
				setTimeout(function () {
					...
					if (self.IO.MasterFilterMode === 'Single') {
						...
						self.RangeAreaChart.InteractivityOptions.MasterFilterMode = 'Off';
					...
					// toggle on
						...
						self.RangeAreaChart.InteractivityOptions.MasterFilterMode = 'Single';
			...
			case 'multipleMasterFilter': {
				...
				setTimeout(function () {
					...
					if (self.IO.MasterFilterMode === 'Multiple') {
						...
						$('#' + self.trackingClearId).removeClass('invisible');
						self.RangeAreaChart.InteractivityOptions.MasterFilterMode = 'Multiple';
			...
			case 'crossFilter': {
				...
				self.RangeAreaChart.IsMasterFilterCrossDataSource = self.IsMasterFilterCrossDataSource;
				gProgressbar.show();
				setTimeout(function () {
					self.functionBinddata = true;
			...
			case 'ignoreMasterFilter': {								
				...
				self.RangeAreaChart.InteractivityOptions.IgnoreMasterFilters = self.IO.IgnoreMasterFilters;
			...
			case 'targetArgument': {
				...
				self.RangeAreaChart.InteractivityOptions.TargetDimensions = 'Argument';
				/* DOGFOOT hsshim 2020-02-06 마스터 필터 기능 리팩토링 적용 (코드 정리) */
				if (self.IO.MasterFilterMode !== 'Off') {
					self.clearTrackingConditions();
					self.selectFirstPoint();
			...
			case 'targetSeries': {
				...
				self.RangeAreaChart.InteractivityOptions.TargetDimensions = 'Series';
				/* DOGFOOT hsshim 2020-02-06 마스터 필터 기능 리팩토링 적용 (코드 정리) */
				if (self.IO.MasterFilterMode !== 'Off') {
					self.clearTrackingConditions();
					self.selectFirstPoint();
		...
		/* DESIGN OPTIONS */
			...
			case 'captionVisible': {
				...
				if (titleBar.css('display') === 'none') {
					...
					self.RangeAreaChart.ShowCaption = true;
				} else {
					titleBar.css('display', 'none');
					self.RangeAreaChart.ShowCaption = false;
				}
				break;
			}
			// edit chart title
...
//                            	goldenLayout.changeTitle(self.itemid, newName, goldenLayout.canvasLayout.root.contentItems);
                            	...
                            	self.RangeAreaChart.Name = newName;
                            	self.Name = newName;
                            	p.hide();
                            }
						});
			...
			case 'rotation': {
				...
				if(self.RangeAreaChart.AxisX.Rotation !== 0 && !self.RangeAreaChart.Rotated){
					WISE.confirm(
						'회전시 X축 기울기가 적용되지<br>않습니다. 회전하시겠습니까?', 
						{
							buttons: {
								confirm: {
									...
									text: '확인',
									action: function() {
										self.RangeAreaChart.Rotated = !(self.RangeAreaChart.Rotated);
										self.dxItem.option('rotated', self.RangeAreaChart.Rotated);
										$AlertPopup.hide();
									}
								},
								cancel: {
				...
				p.option({
					...
					contentTemplate: function(contentElement) {
						...
						$('#' + self.itemid + '_xShow').dxCheckBox({
							value: self.RangeAreaChart.AxisX.Visible != undefined ? self.RangeAreaChart.AxisX.Visible : true ,
							text: 'X축 표시'
						});
						// edit X axis title and label orientation
						$('#' + self.itemid + '_xInputs').dxForm({
							formData: {
								'사용자 정의 텍스트': self.RangeAreaChart.AxisX.Title,
								'기울기': self.RangeAreaChart.AxisX.Rotation
							},
							items: [
								{
                        ...
                        // confirm and cancel
						$('#ok-hide').on('click', function() {
							self.RangeAreaChart.AxisX.Visible = $('#' + self.itemid + '_xShow').dxCheckBox('instance').option('value');
							self.RangeAreaChart.AxisX.Title = $('#' + self.itemid + '_xInputs').dxForm('instance').getEditor('사용자 정의 텍스트').option('value');
							self.RangeAreaChart.AxisX.Rotation = $('#' + self.itemid + '_xInputs').dxForm('instance').getEditor('기울기').option('value');
							if(self.RangeAreaChart.AxisX.Rotation !== 0 && self.RangeAreaChart.Rotated){
								WISE.confirm(
									'회전된 상태에선 기울기가 적용되지 않습니다. 적용하시겠습니까?', 
									{
										buttons: {
											confirm: {
												...
												text: '확인',
												action: function() {
						                            self.dxItem.option('argumentAxis.label.visible', self.RangeAreaChart.AxisX.Visible);
						                            self.dxItem.option('argumentAxis.title', { text: self.RangeAreaChart.AxisX.Title });
						                            if (self.dxItem.option('argumentAxis.label.displayMode') != 'rotate') {
						                                self.dxItem.option('argumentAxis.label.displayMode', 'rotate');
						                            }
						                            self.dxItem.option('argumentAxis.label.rotationAngle', self.RangeAreaChart.AxisX.Rotation);
													self.dxItem.refresh();
						                            p.hide();
													$AlertPopup.hide();
												}
				...
				p.option({
					...
					contentTemplate: function(contentElement) {
							...
							items: [
								{
									...
									editorOptions: {
										...
										value: self.RangeAreaChart.AxisY.FormatType,
								...
								},
								{
									...
									editorOptions: {
										...
										value: self.RangeAreaChart.AxisY.Unit,
										onInitialized: function(e) {
											var formatType = self.RangeAreaChart.AxisY.FormatType;
								...
								},
								{
									...
									editorOptions: {
										value: self.RangeAreaChart.AxisY.ShowZero
									}
								},
								{
									dataField: 'Y축 표시',
									editorType: 'dxCheckBox',
									editorOptions: {
										value: self.RangeAreaChart.AxisY.Visible
									}
								},
								{
									dataField: '사용자 정의 텍스트',
									editorType: 'dxTextBox',
									editorOptions: {
										value: self.RangeAreaChart.AxisY.Title
									}
								},
								{
									...
									editorType: 'dxCheckBox',
									editorOptions: {
										value: self.RangeAreaChart.AxisY.SuffixEnabled,
										onInitialized: function(e) {
											var formatType = self.RangeAreaChart.AxisY.FormatType;
											if (formatType === 'Auto' || formatType === 'General' || formatType === 'Scientific' || formatType === 'Percent') {
												e.component.option('disabled', true);
								...
								},
								{
									...
									editorType: 'dxTextBox',
									editorOptions: {
										value: self.RangeAreaChart.AxisY.MeasureFormat.O,
										onInitialized: function(e) {
											var formatType = self.RangeAreaChart.AxisY.FormatType;
											var suffixEnabled = self.RangeAreaChart.AxisY.SuffixEnabled;
								...
								},
								{
									...
									editorType: 'dxTextBox',
									editorOptions: {
										value: self.RangeAreaChart.AxisY.MeasureFormat.K,
										onInitialized: function(e) {
											var formatType = self.RangeAreaChart.AxisY.FormatType;
											var suffixEnabled = self.RangeAreaChart.AxisY.SuffixEnabled;
											if (formatType === 'Auto' || formatType === 'General' || formatType === 'Scientific' || formatType === 'Percent' || !suffixEnabled) {
								...
								},
								{
									...
									editorType: 'dxTextBox',
									editorOptions: {
										value: self.RangeAreaChart.AxisY.MeasureFormat.M,
										onInitialized: function(e) {
											var formatType = self.RangeAreaChart.AxisY.FormatType;
											var suffixEnabled = self.RangeAreaChart.AxisY.SuffixEnabled;
											if (formatType === 'Auto' || formatType === 'General' || formatType === 'Scientific' || formatType === 'Percent' || !suffixEnabled) {
								...
								},
								{
									...
									editorType: 'dxTextBox',
									editorOptions: {
										value: self.RangeAreaChart.AxisY.MeasureFormat.B,
										onInitialized: function(e) {
											var formatType = self.RangeAreaChart.AxisY.FormatType;
											var suffixEnabled = self.RangeAreaChart.AxisY.SuffixEnabled;
											if (formatType === 'Auto' || formatType === 'General' || formatType === 'Scientific' || formatType === 'Percent' || !suffixEnabled) {
								...
								},
								{
									...
									editorOptions: {
										...
										value: self.RangeAreaChart.AxisY.Precision,
										onInitialized: function(e) {
											var formatType = self.RangeAreaChart.AxisY.FormatType;
								...
								},
								{
									...
									editorOptions: {
										items: ['반올림', '올림', '버림'],
										value: typeof  self.RangeAreaChart.AxisY.PrecisionOption !== 'undefined' ? self.RangeAreaChart.AxisY.PrecisionOption : '반올림',
									}
								},
								{
									dataField: '그룹 구분 포함',
									editorType: 'dxCheckBox',
									editorOptions: {
										value: self.RangeAreaChart.AxisY.Separator,
										onInitialized: function(e) {
											var formatType = self.RangeAreaChart.AxisY.FormatType;
											if (formatType === 'Auto' || formatType === 'General' || formatType === 'Scientific' || formatType === 'Percent') {
												e.component.option('disabled', true);
						...
						$('#ok-hide').off('click').on('click', function() {
							...
							var precisionOption = formData['정도 옵션'];
							var separator = formData['그룹 구분 포함'];
							self.RangeAreaChart.AxisY.FormatType = formatType;
							self.RangeAreaChart.AxisY.Unit = unit;
							self.RangeAreaChart.AxisY.ShowZero = showZero;
							self.RangeAreaChart.AxisY.Visible = visible;
                            self.RangeAreaChart.AxisY.Title = title;
                            /* DOGFOOT ktkang Y축 이름 저장 오류 수정  20200130 */
                            self.RangeAreaChart.AxisY.TitleRename = true;
							self.RangeAreaChart.AxisY.SuffixEnabled = suffixEnabled;
							self.RangeAreaChart.AxisY.MeasureFormat = suffix;
							self.RangeAreaChart.AxisY.Precision = precision;
							self.RangeAreaChart.AxisY.PrecisionOption = precisionOption;
							self.RangeAreaChart.AxisY.Separator = separator;
							// self.dxItem.option('valueAxis[0].title.text', self.RangeAreaChart.AxisY.Title);
							// for (var i = 0; i < self.dxItem.option('valueAxis').length; i++) {
							// 	self.dxItem.option('valueAxis[' + i + '].label.visible', self.RangeAreaChart.AxisY.Visible);
							// 	// if Y axis is not visible, hide title
							// 	var title = self.dxItem.option('valueAxis[' + i + '].title.text');
							// 	self.dxItem.option('valueAxis[' + i + '].title.text', self.RangeAreaChart.AxisY.Visible ? title : '');
							...
							// 	});
							// 	self.dxItem.option('valueAxis[' + i + '].label.showZero', self.RangeAreaChart.AxisY.ShowZero);
							// }
							//2020.02.17 mksong 옵션 사용한 바인드일 때 프로그레스 하이드 되도록 수정 dogfoot
							self.functionBinddata = true;
							self.bindData(self.globalData, true);
...
//						}
						...
						$('#' + self.itemid + '_toggleLegend').dxButton({
                            text: '범례 표시',
							onClick: function() {
								self.RangeAreaChart.ChartLegend.Visible = !self.RangeAreaChart.ChartLegend.Visible;
								self.dxItem.option('legend.visible', self.RangeAreaChart.ChartLegend.Visible);
							}
                        });
                        
						$.each($('.select-position'), function(index, position) {
							if (self.RangeAreaChart.ChartLegend.IsInsidePosition) {
								if (self.RangeAreaChart.ChartLegend.InsidePosition === $(position).data('description')
								       && $(position).data('position') === 'inside') {
									$(position).addClass('on');
									return false;
								}
							} else {
								if (self.RangeAreaChart.ChartLegend.OutsidePosition === $(position).data('description')
						...
						$('.select-position').off('click').on('click', function(e) {
							...
							if (newPosition === 'inside') {
								self.RangeAreaChart.ChartLegend.IsInsidePosition = true;
								self.RangeAreaChart.ChartLegend.InsidePosition = newDescription;
							} else {
								self.RangeAreaChart.ChartLegend.IsInsidePosition = false;
								self.RangeAreaChart.ChartLegend.OutsidePosition = newDescription;
							}
							var newLegend = self.CU.Legend.get(self.RangeAreaChart.ChartLegend);
				...
				p.option({
					...
					contentTemplate: function(contentElement) {
						...
						$.each($('.select-style'), function(index, style) {
							if (self.RangeAreaChart['SeriesType'] === $(style).attr('seriestype').toLowerCase()) {
						...
						$('.select-style').off('click').on('click', function() {
							...
							if (newStyle === 'bubble' || self.RangeAreaChart.SeriesType === 'bubble') {
								self.RangeAreaChart.SeriesType = newStyle;
								self.fieldManager.seriesType = type;
								//2020.02.17 mksong 옵션 사용한 바인드일 때 프로그레스 하이드 되도록 수정 dogfoot
								self.functionBinddata = true;
								...
								self.RangeAreaChart.SeriesType = newStyle;
								self.fieldManager.seriesType = type;
								$.each(WISE.util.Object.toArray(self.RangeAreaChart.Panes.Pane.Series.Simple), function(sIndex, series) {
									series.SeriesType = newStyle;
								});
								$.each(self.dxItem.series, function(index, _series) {
									_series.type = newStyle;
			...
			case 'editPalette': {
				...
				// popup configs
				var p = $('#editPopover').dxPopover('instance');
				var chagePalette = self.RangeAreaChart.Palette;
				p.option({
                    target: '#editPalette',
					onContentReady: function() {
					...
					contentTemplate: function(contentElement) {
                        ...
                        // palette select
                        var originalPalette = paletteCollection.indexOf(self.RangeAreaChart.Palette) != -1
										? self.RangeAreaChart.Palette
										: 'Custom';
						select.dxSelectBox({
                            width: 400,
                        ...
                        $('#save-ok').on('click', function() {
                            self.RangeAreaChart.Palette = paletteObject2[select.dxSelectBox('instance').option('value')];
                        ...
                        $('#save-cancel').on('click', function() {
                            self.dxItem.option('palette', self.RangeAreaChart.Palette);
                            chagePalette = self.RangeAreaChart.Palette;
                            p.option('visible', false);
                        });
					},
					onHiding:function(){
						 self.RangeAreaChart.Palette = chagePalette;
					}
                });
                p.option('visible', !(p.option('visible')));
				break;
				...
				p.option({
					...
					contentTemplate: function(contentElement) {
						...
						$('#ok-hide').on('click', function() {
                            ...
                            self.RangeAreaChart['Palette'] = newPalette;
				...
				p.option({
					...
					contentTemplate: function(contentElement) {
						...
						$('#' + self.itemid + '_editAnimation').dxRadioGroup({
							dataSource: ['없음', '입방', '선형'],
                            value: mapping[self.RangeAreaChart['Animation']],
                            width: 70,
							onValueChanged: function(e) {
								self.RangeAreaChart['Animation'] = mapping[e.value];
								if (e.value === '없음') {
									...
									self.dxItem.option({
										animation: {
											enabled: true,
											easing: self.RangeAreaChart['Animation']
										}
									});
                                }
                                self.dxItem.refresh();
...
WISE.namespace('WISE.libs.Dashboard.item.RangeAreaChart');
WISE.libs.Dashboard.item.RangeAreaChart.Series = function() {
...
WISE.libs.Dashboard.item.RangeAreaChart.Series.prototype = {
	pane: undefined,
	serieses: undefined,
...
WISE.libs.Dashboard.item.RangeAreaChart.SimpleSeries = WISE.util.Object.extend(WISE.libs.Dashboard.item.RangeAreaChart.Series);
WISE.libs.Dashboard.item.RangeAreaChart.SimpleSeries.prototype.seriesType = undefined;
WISE.libs.Dashboard.item.RangeAreaChart.SimpleSeries.prototype.defineCommonOption = function(_S, _dataMember, _series) {
	switch (_series.type) {
	case 'bar':	
	case 'stackedbar':	
	case 'fullstackedbar':	
...
WISE.libs.Dashboard.item.RangeAreaChart.SimpleSeries.prototype.getSeriesListByValues = function() {
	var self = this;
	var seriesList = [];
	var labelList = {};
	var deltaseries = [];
...
WISE.libs.Dashboard.item.RangeAreaChart.SimpleSeries.prototype.getSeriesListBySeriesDimensions = function() {
	var self = this;
	var seriesList = [];
...
WISE.libs.Dashboard.item.RangeAreaChart.RangeSeries = WISE.util.Object.extend(WISE.libs.Dashboard.item.RangeAreaChart.Series);
WISE.libs.Dashboard.item.RangeAreaChart.RangeSeries.prototype.getSeriesListByValues = function() {
	var self = this;
	var seriesList = [];
	_.each(WISE.util.Object.toArray(this.serieses), function(_S) {
		var format = {};
		...
		var series = {
			...
			pane: self.pane.name,
			type: (_S['SeriesType'] || 'rangeArea').toLowerCase(),
...
WISE.libs.Dashboard.item.RangeAreaChart.RangeSeries.prototype.getSeriesListBySeriesDimensions = function() {
	var self = this;
	var seriesList = [];
...
WISE.libs.Dashboard.item.RangeAreaChart.BubbleSeries = WISE.util.Object.extend(WISE.libs.Dashboard.item.RangeAreaChart.Series);
WISE.libs.Dashboard.item.RangeAreaChart.BubbleSeries.prototype.getSeriesListByValues = function() {
	var self = this;
	var seriesList = [];
...
WISE.libs.Dashboard.item.RangeAreaChart.BubbleSeries.prototype.getSeriesListBySeriesDimensions = function() {
	var self = this;
	var seriesList = [];
...
WISE.libs.Dashboard.RangeAreaChartFieldManager = function() {
	var self = this;
...
//		var NumericFormat = {'FormatType' : 'Number', 'IncludeGroupSeparator': true, 'Precision': 0, 'Unit': "Ones"};
		...
		for(var i = 0; i < _fieldlist.length; i++){
			var listType = $(_fieldlist[i]).attr('prev-container').substr(0,$(_fieldlist[i]).attr('prev-container').lastIndexOf('t')+1) == 'rangeareachartValueList' || $(_fieldlist[i]).attr('prev-container').substr(0,$(_fieldlist[i]).attr('prev-container').lastIndexOf('t')+1) == 'rangeareachart_hide_measure_list'|| 
		...
		_.each(_panes,function(_p) {
				...
				if(_p.options ==  undefined){
					...
					var Value = {
						...
						IgnoreEmptyPoints: false,
						ShowPointMarkers: self.panelabeloption[optionItem].showPointMarkers,
						PointLabelOptions: {
