(function () {
            ...
            click: function (evt, cmd) {
                ...
                switch (cmd.commandName) {
                    ...
                    case "format-comma":
                        ...
                        updateFormat(); // update the format text box after click the button.
                        break;
    ...
    function setSpecialClass() {
        ...
        if (designer.res.name === "jp") {
            $("#Textinsertfunction").find(".ui-button-text").append("<span class=\"ui-custom-triangle\"></span>");
        } else {
    ...
    //This logic is from the formatDalog _parseFormat function. The FormatDialog also needs show the Number tab if it's numberFormat.
    function _getDecimalCount(format) {
        var decimalPlace = 0;
        if (format.indexOf('.') !== -1) {
    ...
    function _isNumberFormat(format) {
        var decimalPlace = _getDecimalCount(format);
        var numberFormats = designer.res.numberFormats;
        if (format.indexOf('\\\\') !== -1) { // If format contains \\ it's not built-in format.
            return false;
        }
        var tempFormat;
        if (decimalPlace > 0) {
            var replaceStr = "\\" + format.substring(format.indexOf('.'), format.indexOf('.') + decimalPlace + 1);
            tempFormat = format.replace(new RegExp(replaceStr, 'g'), "").replace(/\\/g, "");
        } else {
            tempFormat = format.replace(/\\/g, "");
        }
        for (var j = 0; j < numberFormats.length; j++) {
            if (tempFormat === numberFormats[j]) {
                return true;
            }
        }
        return false;
    }
    function _isAccountingFormat(format) {
        var decimalPlace = _getDecimalCount(format);
        var replaceStr = "\\" + format.substring(format.indexOf('.'), format.indexOf('.') + decimalPlace + 1);
        ...
        for (var j = 0; j < accountingFormats.length; j++) {
    ...
    function updateFormat() {
        ...
        } else if (_isNumberFormat(style.formatter)) { //To fix SJS-3148, display the Number format correctly.
            setDropDownContent('#number-format', designer.res.commonFormats.Number.label);
        } else if (_isAccountingFormat(style.formatter)) { //To fix SJS-3607, display the Accounting format correctly.
            setDropDownContent('#number-format', designer.res.commonFormats.Accounting.label);
        } else {
