...
public class ConvertSheet2HtmlConverterForPdf {
	private Workbook wb;
	 ...
	 */
	public void main(String[] args, HashMap<String, String> imageMap, String path) {
		if (args.length < 2) {
	...
	public void printPage() throws IOException {
		try {
			...
			if (completeHTML) {
				...
				this.out.format("<html style=\"border:solid 0px #fff;\">%n");
...
//				this.out.format("<meta charset=\"UTF-8\"/>%n");
				this.out.format("</head>%n");
				this.out.format("<body style='font-family: NanumSquare; border:solid 0px #fff;'>%n");
			}
		...
		try {
			while(iterator.hasNext()){
				...
				if(key.equals(sheet.getSheetName())) {
					k = 1;
					String uriEnc = path + value;
//					String uriEnc = this.imageMap.get("contextPath") + "/" + value;
//					uriEnc = uriEnc.replaceAll("\\+", "%20");
//					uriEnc = uriEnc.replaceAll(File.separator+File.separator, File.separator + File.separator + File.separator + File.separator);
//					System.out.println("이미지 태그 : <img src=\"" + uriEnc + "\" alt=\"\" />");
//									uriEnc.replace("%", "%%");
...
//			this.out.format("    <td class=\"%s\">%d</td>%n", ROW_HEAD_CLASS, row.getRowNum() + 1);
			for (int i = firstColumn; i < endColumn; i++) {
				...
				this.out.format("    <td class=\"%s\" %s style=\"border:solid 1px #333\">%s</td>%n", styleName(style), attrs, content);
			}
			this.out.format("  </tr>%n");
		}
		this.out.format("</tbody>%n");
