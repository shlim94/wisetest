 ...
 * 
 * $Id: Base64Encoder.java,v 1.1 2011/08/05 06:36:23 ���ؼ� Exp $
 * 
 */
...
//		System.out.println(requiredSize);
	 ...
	 */
	public static byte[] decode(String value) throws InvalidBase64Exception {
		if (value == null || value.length() == 0)
			return new byte[0];

		// The length of the string must be a multiple of 4.
		if ((value.length() % 4) != 0)
			throw new InvalidBase64Exception("The length of a Base64 string must be a multiple of 4.");
		...
		for (int i = 0; i < value.length(); i++) {
			...
			throw new InvalidBase64Exception("An invalid character is found in the Base64 string.");
		}
