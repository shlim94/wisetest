...
//            	var path = WISE.Constants.context+"/resources/shp2geojson/shapefile.js"
...
//        }
        ...
        reader.onload = (function(that){
            return function(e){
                self.onFileLoad(e.target.result);
                self.ready = true;
            }
        })(this);
    ...
    this._readBounds = function(object){
        var s = this.stream
    ...
    this._readParts = function(record){
        var s = this.stream,
            nparts,
            parts = []
    ...
    this._readPoints = function(record){
        var s = this.stream,
            points = [],
            npoints = record.numPoints || (record.numPoints = s.readSI32())
    ...
    this._readPolygon = function(record){
        var s = this.stream
...
//            header = this.header = {}
        ...
        // File code (always hex value 0x0000270a)
        self.header.fileCode = s.readSI32(true)

        if(self.header.fileCode != parseInt(0x0000270a))
            throw "Invalid File Code"
        ...
        // File length (in 16-bit words, including the header)
        self.header.fileLength = s.readSI32(true) * 2

        self.header.version = s.readSI32()

        self.header.shapeType = SHAPE_TYPES[s.readSI32()]

        // Minimum bounding rectangle (MBR) of all shapes contained within the
		// shapefile; four doubles in the following order: min X, min Y, max X,
		// max Y
        this._readBounds(self.header)
...
//            records = this.records = [],
    ...
    this.formatIntoGeoJson = function(){
        var bounds = self.header.bounds,
            records = self.records,
            features = [],
            feature, geometry, points, fbounds, gcoords, parts, point;
//            geojson = {}
            ...
            switch (record.shapeType) {
                ...
                case "Polygon":
                    ...
                    for (var pt = 0; pt < parts.length; pt++){
                        ...
                        // partIndex 0 == main poly, partIndex > 0 == holes in
						// poly
                        for (var p = partIndex; p < (parts[pt+1] || points.length); p++){
                            point = points[p]
                            part.push([point.x,point.y])
    ...
    this.addDBFDataToGeoJSON = function(){
        var features = self.geojson.features,
            len = features.length,
            records = self.dbf.records
