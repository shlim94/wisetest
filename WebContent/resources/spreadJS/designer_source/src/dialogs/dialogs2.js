(function () {
    ...
    var DataValidationDialog = (function (_super) {
        ...
        DataValidationDialog.prototype._applySetting = function () {
            ...
            this._iconImg[0].src = "";
            this._iconImg.hide();
            var defaultDataValidator = new Sheets.DataValidation.DefaultDataValidator();
            var propertyArray = ["type", "showInputMessage", "inputTitle", "inputMessage", "showErrorMessage", "errorTitle", "errorMessage", "errorStyle", "ignoreBlank", "inCellDropdown", "comparisonOperator"]
            var isBlankValidator = true;
            if (dv) {
                for (var i = 0; i < propertyArray.length; i++) {
                    var propertyFun = propertyArray[i];
                    if (dv[propertyFun]() !== defaultDataValidator[propertyFun]()) {
                        isBlankValidator = false;
                        break;
                    }
                }
                if (!isBlankValidator || JSON.stringify(dv.highlightStyle()) !== JSON.stringify(defaultDataValidator.highlightStyle())) {
                    isBlankValidator = false;
                }
            }
            if (isBlankValidator) {
                designer.actions.doAction("setDataValidation", designer.wrapper.spread, null);
            } else {
