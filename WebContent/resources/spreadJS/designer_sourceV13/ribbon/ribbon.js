(function () {
    ...
    function initRibbonBar() {
        ...
        var menuConfigJson;
        if(parent.menuConfigManager){
            menuConfigJson = parent.menuConfigManager.getMenuConfig.Menu.SPREAD_MENU_TYPE;
        }else{
        	menuConfigJson =         	
        	'{"Menu" :'+
		    '{'+
        	...
        	menuConfigJson = JSON.parse(menuConfigJson).Menu.SPREAD_MENU_TYPE;        	
        }
        
        if(menuConfigJson){
        	if(!menuConfigJson.Print){
            	$('#printTab').css('display', 'none');
            }
            /*dogfoot 바인드 유형 환경설정 값 따라가도록 변경 shlim 20200728*/
        	 if(typeof menuConfigJson.BindType != undefined){
             	parent.gDashboard.queryHandler.spreadtable = menuConfigJson.BindType == "tableBind" ? true : false;
             }
        }
                ...
                switch (cmd.commandName) {
            		...
            		case "downloadReport":
            		    if($('#downloadMenu').css('display') == 'none'){
            		    	...
            		    	$('#downloadReport .ui-button-text').addClass('on');  
            		    }else{
            		    	$('#downloadMenu').css('display', 'none');
            		    	$('#downloadReport .ui-button-text').removeClass('on');   
            		    }

        		        if($('#downloadMenu').css('display') == 'block'){
        		        	
        		        	var thisW = $('#downloadReport').position().left + $('#downloadReport').outerWidth();
        		            $('#downloadMenu').css({'left' : thisW});
        		            var thisH = $('#downloadReport').position().top;
        		            $('#downloadMenu').css({'top' : thisH});
        		            
        		            $('html').click(function(e) {
        		                if($(e.target).attr('id') !== 'downloadMenu') {
        		                	$('#downloadMenu').css('display', 'none');
        		                	$('#downloadReport .ui-button-text').removeClass('on');       
        		                }
        		            });
        		        ...
        		        $('.right-type').find('.more-link').each(function(){
        		            var lnbthisW = $(this).outerWidth();
        		            $(this).css({'right' : - lnbthisW});
        		            // var lnbbtnOffsetT = $(this).offset().top;
        		            // $(this).offset( { top: lnbbtnOffsetT } );
        		        });
        		        
        		        $('#downloadReportXLSX').on('click', function(){
        		        	document.getElementById('export-excel').click();
        		        })
        		        $('#downloadReportCSV').on('click', function(){
        		        	document.getElementById('export-csv').click();
        		        })
        		        $('#downloadReportPDF').on('click', function(){
