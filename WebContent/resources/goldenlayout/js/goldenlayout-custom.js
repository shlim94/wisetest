/**
 * Modified for WISE Intelligence
 */
	...
	function( callback ) {
		...
		if(gDashboard.reportType == 'AdHoc') {
        	$.each(gDashboard.itemGenerateManager.dxItemBasten, function(_i, _e) {
        		if(_e.type == 'PIVOT_GRID' && typeof _e.filteredData != 'undefined' && _e.filteredData.length > userJsonObject.searchLimitRow) {
        			$("#" + _e.itemid).parent().css('width', '0px');
					$("#" + _e.itemid).closest('.dashboard-item').addClass('pivot-scroll');
        			_e.dxItem.option('height', 'auto');
        		}
        	});
        }
	} );
...
lm.utils.copy( lm.utils.DragListener.prototype, {
	...
	onMouseDown: function( oEvent ) {
		...
		/* DOGFOOT ktkang 뷰어에서 골든레이아웃 드래그해서 이동 못하게하는 기능   20200212 */
		//2020.02.13 mksong 비정형 뷰어에서 레이아웃 변동 가능하도록 수정 dogfoot
		if((oEvent.button == 0 || oEvent.type === "touchstart")) {
			if(gDashboard.reportType == 'AdHoc'){
				...
				if(WISE.Constants.editmode != 'viewer'){
					...
					if(this._eElement[0].className.indexOf('lm_tab') == -1){
						var coordinates = this._getCoordinates( oEvent );
		...
		//2020.02.13 mksong 비정형 뷰어에서 레이아웃 변동 가능하도록 수정 끝 dogfoot
	},
...
lm.utils.copy( lm.LayoutManager.prototype, {
	...
	updateSize: function( width, height ) {
		if( arguments.length === 2 ) {
			...
			// ChangeReportType 이후 resize를 하는데 이에 영향받아서 사이즈 조절 해줘야함
			if(gDashboard.structure.Layout == 'GTCB' && !gDashboard.isSingleView){
 ...
 */
lm.controls.DragProxy = function( x, y, dragListener, layoutManager, contentItem, originalParent, itemId ) {
...
lm.utils.copy( lm.controls.DragProxy.prototype, {
	...
	_setDropPosition: function( x, y ) {
		...
		if( this._area !== null ) {
			if (this._itemId === 'container' && !this._area.contentItem.isRoot && !this._area.contentItem.config.name) {
				this._area = this._lastValidArea || this._area;
			} else if (this._itemId !== 'container' && !this._area.contentItem.isRoot && !this._area.contentItem.config.contentId && !this._area.contentItem.config.id) {
				this._area = this._lastValidArea || this._area;
			} else {
				this._lastValidArea = this._area;
				this._area.contentItem._$highlightDropZone( x, y, this._area, this);
			}
		}
	},
	...
	_setDimensions: function() {
		...
		this._contentItem.element.show();
...
lm.controls.Header = function( layoutManager, parent ) {
	...
	if(parent.config.nTabHdVisible){
	    this.element = $( lm.controls.Header._tab_template );	
	}else{
...
/*dogfoot 컨테이너/아이템 헤더 구분 기능 추가 shlim 20200918*/
lm.controls.Header._tab_template = [
	'<div class="lm_header tab_header">',
	'<ul class="lm_tabs"></ul>',
	'<ul class="lm_controls"></ul>',
	'<ul class="lm_tabdropdown_list"></ul>',
	'</div>'
...
lm.utils.copy( lm.controls.Header.prototype, {
	...
	_createControls: function() {
		...
		if( this._getHeaderSetting( 'maximise' ) ) {
			...
			/* DOGFOOT ktkang KERIS 영어 한글화   20200212 */
			maximiseLabel = '최대화';
			minimiseLabel = '최소화';
...
//			}
			...
			if(tabElement.hasClass("lm_tab tab_cont_box_top_tit")){
            	if(tabElement.attr('id').indexOf("TabPage") >-1){
	            	/*dogfoot 탭 적을때 탭 크기 설정 20210728*/
            		var tabWidthSize = ((availableWidth-50) / (this.tabs.length)) - 16
            		
            		if(tabWidthSize >= 150){
            			tabWidthSize = 150
            		}
            		
            		tabElement.width(tabWidthSize);
						hasVisibleTab = true;
						this._lastVisibleTabIndex = i;
						tabElement.removeData( 'lastTabWidth' );
						this.tabsContainer.append( tabElement );
            		...
            		if( totalTabWidth > availableWidth && hasVisibleTab ) {
						tabElement.data( 'lastTabWidth', tabWidth );
						$('.lm_tabdropdown_list_Popup').append( tabElement );
					}
					else {
						hasVisibleTab = true;
						this._lastVisibleTabIndex = i;
...
lm.utils.copy( lm.items.AbstractContentItem.prototype, {
	...
	removeChild: function( contentItem, keepChild ) {
		...
		if( this.contentItems.length > 0 ) {
			 ...
			 */
		} else if( !(this instanceof lm.items.Root) && this.config.isClosable === true && this.config.id !== 'container' ) {
	...
	_createContentItems: function( config ) {
		...
		for( i = 0; i < config.content.length; i++ ) {
			...
			if(config.content[ i ].componentState != undefined){
				$(oContentItem).attr('id',config.content[ i ].componentState.item);	
			}
...
lm.utils.copy( lm.items.Root.prototype, {
	...
	_$onDrop: function( contentItem, area ) {
		/**
		 * EDIT: Always wrap content items in a stack
		 */
		var stack = this.layoutManager.createContentItem( {
			type: 'stack',
			header: contentItem.config.header || {},
			enableHeaderDrop: contentItem.isStack,
			contentId: contentItem.isComponent ? contentItem.config.id : undefined,
			name: contentItem.config.number ? 'tabContainerDashboardItem' + contentItem.config.number : undefined,
			number: contentItem.config.number ? contentItem.config.number : undefined
		}, this );
		stack._$init();
...
lm.utils.copy( lm.items.Stack.prototype, {
	...
	/* DOGFOOT ktkang KERIS EDS포탈에서 보여주는 보고서 체크 기능  20200205 */
		...
		var contentHeight = this.element.height() - (!this._sided ? headerSize : 0) + num;

		this.childElementContainer.width( contentWidth );
		this.childElementContainer.height( contentHeight );

		for( i = 0; i < this.contentItems.length; i++ ) {
			/* DOGFOOT syjin 뷰어에서 대시보드 차트 x축 짤림현상 수정  20211126 */
			//if(typeof this.contentItems[i].id != 'undefined' && this.contentItems[i].id.indexOf('chartDashboard') > -1 && gDashboard.reportType == 'AdHoc') {
			//if(typeof this.contentItems[i].id != 'undefined' && this.contentItems[i].id.indexOf('chartDashboard') > -1) {
			//	this.contentItems[ i ].element.width( contentWidth ).height( contentHeight - 40);
			//} else {
	...
	_$onDrop: function( contentItem ) {
		...
		if( this._dropSegment === 'header') {
			if (this.config.enableHeaderDrop) {
	 ...
	 * @returns {void}
	 */
	_$highlightDropZone: function( x, y, _area, _item) {
		var segment, area;
		...
		for( segment in this._contentAreaDimensions ) {
			...
			if( area.x1 < x && area.x2 > x && area.y1 < y && area.y2 > y ) {
				...
				if( segment === 'header') {
					if (this.config.enableHeaderDrop && !(_item._contentItem && _item._contentItem.isComponent)) {
	...
	_setupHeaderPosition: function() {
		...
		if(this.contentItems[0] != undefined){
			if(this.contentItems[0].id != undefined){
				this.element[0].id = this.contentItems[0].id; 
			}			
		}
