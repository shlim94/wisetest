WISE.libs.Dashboard.item.CoordinateLineGenerator = function() {
	var self = this;

	this.type = 'COORDINATE_LINE';
...
//				.toArray(_item.DataItems.Dimension) : [];
	...
	this.setCoordinateLine = function(){
		this.fieldManager.init();
		gDashboard.itemGenerateManager.getherFields(this.fieldManager, self.type);
		this.CoordinateLine['ComponentName'] = this.ComponentName;
		this.CoordinateLine['Name'] = this.Name;
		this.CoordinateLine['DataSource'] = this.dataSourceId;
		
		this.CoordinateLine['DataItems'] = this.fieldManager.DataItems;
		this.CoordinateLine['Arguments'] = this.fieldManager.Arguments;
		this.CoordinateLine['Values'] = this.fieldManager.Values;
	
		this.measures = this.fieldManager.measures;
		this.dimensions = this.fieldManager.dimensions;
		
		this.meta = this.CoordinateLine;
		
		if (!(this.CoordinateLine['Palette'])) {
			this.CoordinateLine['Palette'] = userJsonObject.userPalette ? userJsonObject.userPalette : userJsonObject.defaultPalette;
		}
		// 20200901 ajkim 마스터 필터 추가 dogfoot
		if (this.CoordinateLine.InteractivityOptions) {
			if (!(this.CoordinateLine.InteractivityOptions.MasterFilterMode)) {
				this.CoordinateLine.InteractivityOptions.MasterFilterMode = 'Off';
			}
			if (!(this.CoordinateLine.InteractivityOptions.TargetDimensions)) {
				this.CoordinateLine.InteractivityOptions.TargetDimensions = 'Argument';
			}
			if (!(this.CoordinateLine.InteractivityOptions.IgnoreMasterFilters)) {
				this.CoordinateLine.InteractivityOptions.IgnoreMasterFilters = false;
			}
		} else {
			this.CoordinateLine.InteractivityOptions = {
		...
		if (!(this.CoordinateLine.AxisY)) {
			this.CoordinateLine.AxisY = {
				FormatType: 'Number',
				Unit: 'Ones',
				ShowZero: true,
				Visible: true,
		...
		if (!(this.CoordinateLine.AxisX)) {
			this.CoordinateLine.AxisX = {
				FormatType: 'Number',
				Unit: 'Ones',
				ShowZero: true,
				Visible: true,
		...
		if (!(this.CoordinateLine['Legend'])) {
			this.CoordinateLine['Legend'] = {
					Visible : true,
					Position : "TopRightVertical"
			}
		}
		
		if(!this.CoordinateLine.LayoutOption){
			this.CoordinateLine.LayoutOption = {
					Legend : {
						family: '맑은 고딕',
						size: 12,
						color: '#000000'
		...
		if(!this.CoordinateLine['ZoomAble']){
			this.CoordinateLine.ZoomAble = 'none'
		}
		
	};
	
	this.setCoordinateLineforOpen = function(){
		if(typeof this.meta == 'undefined'){
			this.setCoordinateLine();
		}
		else{
			this.CoordinateLine = this.meta;
		}
		if(typeof this.meta.DataSource == 'undefined'){
			this.meta['DataSource'] = 'dataSource1';
		}
		if(this.fieldManager != undefined){
			this.CoordinateLine['DataItems'] = this.meta['DataItems'] = this.fieldManager.DataItems;
			this.CoordinateLine['Arguments'] = this.meta['Arguments'] = this.fieldManager.Arguments;
			this.CoordinateLine['Values'] = this.meta['Values'] = this.fieldManager.Values;
			this.measures = this.fieldManager.measures;
			this.dimensions = this.fieldManager.dimensions;
		}
		...
		if (!(this.CoordinateLine['Palette'])) {
			...
			if(gDashboard != undefined){
				var CoordinateLineOption = typeof gDashboard.structure.MapOption.DASHBOARD_XML == 'undefined' ? '' : WISE.util.Object.toArray(gDashboard.structure.MapOption.DASHBOARD_XML.COORDINATE_LINE_DATA_ELEMENT);
				
				$.each(CoordinateLineOption,function(_i,_CoordinateLineOption){
					var CtrlNM;
					if (page[page.length - 1] === 'viewer.do'){
						CtrlNM = _CoordinateLineOption.CTRL_NM +"_"+WISE.Constants.pid;
					}else{
						CtrlNM = _CoordinateLineOption.CTRL_NM;
					}
					if(self.CoordinateLine.ComponentName == CtrlNM){
						self.CoordinateLine['Palette'] = _CoordinateLineOption.PALETTE_NM;
						return false;
					}
				})
				if(typeof this.meta.ColorSheme != 'undefined'){
					var colorList = WISE.util.Object.toArray(this.meta.ColorSheme.ColorSheme);
					this.CoordinateLine.Palette = new Array();
					var newPalette = [];
					$.each(colorList,function(_i,_list){
						self.CoordinateLine.Palette.push(gDashboard.itemColorManager.getHexColor(_list.Color)); 
	...
	this.setCoordinateLineForViewer = function(){
		if(typeof this.meta == 'undefined'){
			this.setCoordinateLine();
		}
		else{
...
//					}
	...
	this.bindData = function(_data) {
		...
		if(this.fieldManager !=null && gDashboard.isNewReport == true){ // 신규 생성
			self.setCoordinateLine();
			gDashboard.itemGenerateManager.addParentItems(self);
			gDashboard.itemGenerateManager.itemCustomize(self,self.CoordinateLine);
			gDashboard.itemGenerateManager.generateItem(self, self.CoordinateLine);
		}else if(this.fieldManager !=null && gDashboard.isNewReport != true){
			gDashboard.itemGenerateManager.getherFields(this.fieldManager, self.type);
			this.setCoordinateLineforOpen();
			gDashboard.itemGenerateManager.addParentItems(self);
			gDashboard.itemGenerateManager.itemCustomize(self,self.CoordinateLine);
			gDashboard.itemGenerateManager.generateItem(self, self.CoordinateLine);
		}
		//d3 뷰어모드 
		else if(self.meta && $.isEmptyObject(self.CoordinateLine)) {
			this.setCoordinateLineForViewer();
			gDashboard.itemGenerateManager.itemCustomize(self,self.CoordinateLine);
			gDashboard.itemGenerateManager.generateItem(self, self.CoordinateLine);
		...
		gDashboard.d3Manager.getDataSource(self, self.type, {'data' : _data, 'measerKey': []}, self.fCoordinateLine);
		//self.fCoordinateLine(_data, this.measures, this.dimensions, self.deleteDuplecateData(_data,[]));
...
//			self.fCoordinateLine(self.filteredData, self.measures, self.dimensions, 
...
//	};
	...
	this.fCoordinateLine = function(jsonData, measures, dimensions, dupleData) {
...
//		.attr("transform", "rotate(-90)")
		...
		.attr("d", function(d){
			return d3.line()
			.x(function(d) { return x(d[dimensions[0].caption]); })
			.y(function(d) { return y(d[dimensions[1].caption]); })
			(d.values)
		});
...
WISE.libs.Dashboard.CoordinateLineFieldManager = function() {
	var self = this;
