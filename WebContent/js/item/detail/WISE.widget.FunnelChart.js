WISE.libs.Dashboard.item.FunnelChartGenerator = function() {
	var self = this;

	this.type = 'FUNNEL_CHART';
...
//						return ret;
				...
				label: {
		            ...
		            position: self.FunnelChart.LabelPosition === "Inside"? 'inside' : self.FunnelChart.LabelPosition,
		            backgroundColor : "none",
            		font : {
		            	color : self.FunnelChart.LabelPosition === "Inside" || self.FunnelChart.LabelPosition === "inside"? "black": undefined,
		            },
...
//		                    e.item.argument + "</span>";
	...
	this.setDrillDownData = function(_dimKey) {
		...
		if(self.TreeMap && self.TreeMap.FilterString && self.TreeMap.FilterString.length > 0) {
			newDataSource.filter(self.FunnelChart.FilterString);
			newDataSource.load();
		}
	...
	this.terminateDrillDownOperation = function() {
		...
		self.FunnelChart.InteractivityOptions.IsDrillDownEnabled = false;
		...
		if(self.FunnelChart && self.FunnelChart.FilterString && self.FunnelChart.FilterString.length > 0) {
			newDataSource.filter(self.FunnelChart.FilterString);
			newDataSource.load();
		}
		self.dxItem.option('dataSource', newDataSource);
	...
	this.setFunnelChart = function(){
		this.fieldManager.init();
		gDashboard.itemGenerateManager.getherFields(this.fieldManager, self.type);
		this.FunnelChart['ComponentName'] = this.ComponentName;
		this.FunnelChart['Name'] = this.Name;
		this.FunnelChart['DataSource'] = this.dataSourceId;

		this.FunnelChart['DataItems'] = this.fieldManager.DataItems;
		this.FunnelChart['Arguments'] = this.fieldManager.Arguments;
		this.FunnelChart['Values'] = this.fieldManager.Values;

		this.measures = this.fieldManager.measures;
		this.dimensions = this.fieldManager.dimensions;

		this.meta = this.FunnelChart;
		//초기 팔레트값 설정
		if (!(this.FunnelChart['Palette'])) {
			this.FunnelChart['Palette'] = userJsonObject.userPalette ? userJsonObject.userPalette : userJsonObject.defaultPalette;
		}
		if(!this.FunnelChart['LabelPosition']){
			this.FunnelChart.LabelPosition = 'Inside';
		}
		
		if (this.FunnelChart.InteractivityOptions) {
			if (!(this.FunnelChart.InteractivityOptions.MasterFilterMode)) {
				this.FunnelChart.InteractivityOptions.MasterFilterMode = 'Off';
			}
			if (!(this.FunnelChart.InteractivityOptions.IsDrillDownEnabled)) {
				this.FunnelChart.InteractivityOptions.IsDrillDownEnabled = false;
			}
			if (!(this.FunnelChart.InteractivityOptions.IgnoreMasterFilters)) {
				this.FunnelChart.InteractivityOptions.IgnoreMasterFilters = false;
			}
		} else {
			this.FunnelChart.InteractivityOptions = {
				MasterFilterMode: 'Off',
				IsDrillDownEnabled: false,
				IgnoreMasterFilters: false
			};
		}
		if (!(this.FunnelChart.IsMasterFilterCrossDataSource)) {
			this.FunnelChart.IsMasterFilterCrossDataSource = false;
		}
		if (!(this.FunnelChart.FilterString)) {
			this.FunnelChart.FilterString = [];
		}else{
			this.FunnelChart.FilterString = JSON.parse(JSON.stringify(this.FunnelChart.FilterString).replace(/"@null"/gi,null));
		}
		
		
		if(!this.FunnelChart.LabelContentType){
			this.FunnelChart.LabelContentType = 'ArgumentAndPercent';
			this.FunnelChart.LabelMeasureFormat = 'O';
			this.FunnelChart.LabelPrefixEnabled = false;
			this.FunnelChart.LabelPrefixFormat = '';
			this.FunnelChart.LabelSuffixEnabled = false;
			this.FunnelChart.LabelSuffix = {
				O: '',
				K: 'K',
				M: 'M',
				B: 'B'
			};
			this.FunnelChart.LabelPrecision = 0;
			this.FunnelChart.LabelPrecisionOption = '반올림';
		}
		if(!this.FunnelChart.TooltipContentType){
			this.FunnelChart.TooltipContentType = 'ArgumentValueAndPercent';
			this.FunnelChart.TooltipMeasureFormat = 'O';
			this.FunnelChart.TooltipPrefixEnabled = false;
			this.FunnelChart.TooltipPrefixFormat = '';
			this.FunnelChart.TooltipSuffixEnabled = false;
			this.FunnelChart.TooltipSuffix = {
				O: '',
				K: 'K',
				M: 'M',
				B: 'B'
			};
			this.FunnelChart.TooltipPrecision = 0;
			this.FunnelChart.TooltipPrecisionOption = '반올림';
		}
		
		if(!this.FunnelChart.Legend){
			this.FunnelChart.Legend = {
					Visible : true,
	...
	this.setFunnelChartforOpen = function(){
		if(typeof this.meta == 'undefined'){
			this.setFunnelChart();
		}
		else{
			this.FunnelChart = this.meta;
		}
		if(typeof this.meta.DataSource == 'undefined'){
			this.meta['DataSource'] = 'dataSource1';
		}
		if(this.fieldManager != undefined){
			this.FunnelChart['DataItems'] = this.meta['DataItems'] = this.fieldManager.DataItems;
			this.FunnelChart['Arguments'] = this.meta['Arguments'] = this.fieldManager.Arguments;
			this.FunnelChart['Values'] = this.meta['Values'] = this.fieldManager.Values;
			this.measures = this.fieldManager.measures;
			this.dimensions = this.fieldManager.dimensions;
		}
		...
		if (!(this.FunnelChart['Palette'])) {
			...
			if(gDashboard != undefined){
				var FunnelChartOption = typeof gDashboard.structure.MapOption.DASHBOARD_XML == 'undefined' ? '' : WISE.util.Object.toArray(gDashboard.structure.MapOption.DASHBOARD_XML.FUNNEL_CHART_DATA_ELEMENT);
				
				$.each(FunnelChartOption,function(_i,_FunnelChartOption){
					var CtrlNM;
					if (page[page.length - 1] === 'viewer.do'){
						CtrlNM = _FunnelChartOption.CTRL_NM +"_"+WISE.Constants.pid;
					}else{
						CtrlNM = _FunnelChartOption.CTRL_NM;
					}
					if(self.FunnelChart.ComponentName == CtrlNM){
						self.FunnelChart['Palette'] = _FunnelChartOption.PALETTE_NM;
						return false;
					}
				})
				if(typeof this.meta.ColorSheme != 'undefined'){
					var colorList = WISE.util.Object.toArray(this.meta.ColorSheme.ColorSheme);
					this.FunnelChart.Palette = new Array();
					var newPalette = [];
					$.each(colorList,function(_i,_list){
						self.FunnelChart.Palette.push(gDashboard.itemColorManager.getHexColor(_list.Color)); 
		...
		var chartDataElements = WISE.util.Object.toArray(dashboardXml.FUNNEL_CHART_DATA_ELEMENT);
		...
		if(!this.FunnelChart['LabelPosition']){
			this.FunnelChart.LabelPosition = 'Columns';
		}
		
		if (typeof this.FunnelChart.LabelContentType === 'undefined') {
			this.FunnelChart.LabelContentType = chartDataElement.LABEL_OPTIONS
				? chartDataElement.LABEL_OPTIONS.CONTENT_TYPE
				: 'ArgumentAndPercent';
		}
		if (typeof this.FunnelChart.LabelMeasureFormat === 'undefined') {
			this.FunnelChart.LabelMeasureFormat = chartDataElement.LABEL_OPTIONS 
				? chartDataElement.LABEL_OPTIONS.MEASURE 
				: 'O';
		}
		if (typeof this.FunnelChart.LabelPrefixEnabled === 'undefined') {
			this.FunnelChart.LabelPrefixEnabled = chartDataElement.LABEL_OPTIONS
				&& chartDataElement.LABEL_OPTIONS.PREFIX_ENABLED_YN === 'Y';
		}
		if (typeof this.FunnelChart.LabelPrefixFormat === 'undefined') {
			this.FunnelChart.LabelPrefixFormat = chartDataElement.LABEL_OPTIONS
				? chartDataElement.LABEL_OPTIONS.PREFIX_FORMAT
				: '';
		}
		if (typeof this.FunnelChart.LabelSuffixEnabled === 'undefined') {
			this.FunnelChart.LabelSuffixEnabled = chartDataElement.LABEL_OPTIONS 
				&& chartDataElement.LABEL_OPTIONS.SUFFIX_ENABLED_YN === 'Y';
		}
		if (typeof this.FunnelChart.LabelSuffix === 'undefined') {
			this.FunnelChart.LabelSuffix = chartDataElement.LABEL_OPTIONS 
				? {
		...
		if (typeof this.FunnelChart.LabelPrecision === 'undefined') {
			this.FunnelChart.LabelPrecision = chartDataElement.LABEL_OPTIONS 
				? chartDataElement.LABEL_OPTIONS.PRECISION 
				: 0;
		}
		if (typeof this.FunnelChart.LabelPrecisionOption === 'undefined') {
			this.FunnelChart.LabelPrecisionOption = chartDataElement.LABEL_OPTIONS 
				? chartDataElement.LABEL_OPTIONS.PRECISION_OPTION 
				: '반올림';
		}
		if (typeof this.FunnelChart.LabelPosition === 'undefined') {
			this.FunnelChart.LabelPosition = 'Columns';
		}
		if (typeof this.FunnelChart.TooltipContentType === 'undefined') {
			this.FunnelChart.TooltipContentType = chartDataElement.TOOLTIP_OPTIONS
				? chartDataElement.TOOLTIP_OPTIONS.CONTENT_TYPE
				: 'ArgumentAndPercent';
		}
		if (typeof this.FunnelChart.TooltipMeasureFormat === 'undefined') {
			this.FunnelChart.TooltipMeasureFormat = chartDataElement.TOOLTIP_OPTIONS 
				? chartDataElement.TOOLTIP_OPTIONS.MEASURE 
				: 'O';
		}
		if (typeof this.FunnelChart.TooltipPrefixEnabled === 'undefined') {
			this.FunnelChart.TooltipPrefixEnabled = chartDataElement.TOOLTIP_OPTIONS
				&& chartDataElement.TOOLTIP_OPTIONS.PREFIX_ENABLED_YN === 'Y';
		}
		if (typeof this.FunnelChart.TooltipPrefixFormat === 'undefined') {
			this.FunnelChart.TooltipPrefixFormat = chartDataElement.TOOLTIP_OPTIONS
				? chartDataElement.TOOLTIP_OPTIONS.PREFIX_FORMAT
				: '';
		}
		if (typeof this.FunnelChart.TooltipSuffixEnabled === 'undefined') {
			this.FunnelChart.TooltipSuffixEnabled = chartDataElement.TOOLTIP_OPTIONS
				&& chartDataElement.TOOLTIP_OPTIONS.SUFFIX_ENABLED_YN === 'Y';
		}
		if (typeof this.FunnelChart.TooltipSuffix === 'undefined') {
			this.FunnelChart.TooltipSuffix = chartDataElement.TOOLTIP_OPTIONS 
				? {
		...
		if (typeof this.FunnelChart.TooltipPrecision === 'undefined') {
			this.FunnelChart.TooltipPrecision = chartDataElement.TOOLTIP_OPTIONS 
				? chartDataElement.TOOLTIP_OPTIONS.PRECISION
				: 0;
		}
		if (typeof this.FunnelChart.TooltipPrecisionOption === 'undefined') {
			this.FunnelChart.TooltipPrecisionOption = chartDataElement.TOOLTIP_OPTIONS 
		...
		if (this.FunnelChart.InteractivityOptions) {
			...
			if (!(this.FunnelChart.InteractivityOptions.TargetDimensions)) {
				this.FunnelChart.InteractivityOptions.TargetDimensions = 'Argument';
			}
			if (!(this.FunnelChart.InteractivityOptions.IsDrillDownEnabled)) {
				this.FunnelChart.InteractivityOptions.IsDrillDownEnabled = false;
			}
	...
	this.setFunnelChartForViewer = function(){
		if(typeof this.meta == 'undefined'){
			this.setFunnelChart();
		}
		else{
...
//		}
		...
		if(this.fieldManager !=null && gDashboard.isNewReport == true){ // 신규 생성
			...
			gDashboard.itemGenerateManager.itemCustomize(self,self.FunnelChart);
			gDashboard.itemGenerateManager.generateItem(self, self.FunnelChart);
		}else if(this.fieldManager !=null && gDashboard.isNewReport != true){
			gDashboard.itemGenerateManager.getherFields(this.fieldManager, self.type);
			this.setFunnelChartforOpen();
			gDashboard.itemGenerateManager.addParentItems(self);
			gDashboard.itemGenerateManager.itemCustomize(self,self.FunnelChart);
			gDashboard.itemGenerateManager.generateItem(self, self.FunnelChart);
		}
		//d3 뷰어모드 
		else if(self.meta && $.isEmptyObject(self.FunnelChart)) {
			this.setFunnelChartForViewer();
			gDashboard.itemGenerateManager.itemCustomize(self,self.FunnelChart);
			gDashboard.itemGenerateManager.generateItem(self, self.FunnelChart);
...
//	this.resize = function() {
//		if($("#" + self.itemid).length != 0){
//			self.fFunnelChart(self.filteredData, self.measures, self.dimensions, self.deleteDuplecateData(self.filteredData,self.measures[0]));	
//		}
//		
//		gProgressbar.hide();
//	};
		...
		/* DESIGN OPTIONS */
			...
			case 'captionVisible': {
				...
				if (titleBar.css('display') === 'none') {
					...
					self.FunnelChart['ShowCaption'] = true;
				} else {
					titleBar.css('display', 'none');
					self.FunnelChart['ShowCaption'] = false;
				}
				break;
			}
			// edit chart title
...
//                            	goldenLayout.changeTitle(self.itemid, newName, goldenLayout.canvasLayout.root.contentItems);
			...
			case 'editPalette': {
				...
				var chagePalette = self.FunnelChart.Palette;
				var firstPalette = self.FunnelChart.Palette;
				p.option({
                    target: '#editPalette',
                    onContentReady: function() {
					...
					contentTemplate: function(contentElement) {
                        ...
                        // palette select
                        var originalPalette = paletteCollection.indexOf(self.FunnelChart.Palette) != -1
										? self.FunnelChart.Palette
										: 'Custom';
						select.dxSelectBox({
                            width: 400,
							...
							onValueChanged: function(e) {
								if (e.value == '사용자 정의 테마') {
                                    self.isCustomPalette = true;
                                    self.dxItem.option('palette', self.FunnelChart.Palette);
                                    
								} else {
                                    self.isCustomPalette = false;
                                    self.FunnelChart.Palette = paletteObject2[e.value];
                                    self.dxItem.option('palette', self.FunnelChart.Palette);
                        ...
                        $('#save-ok').on('click', function() {
                            self.FunnelChart.Palette = paletteObject2[select.dxSelectBox('instance').option('value')];
                        ...
                        $('#save-cancel').on('click', function() {
                        	self.FunnelChart.Palette = firstPalette;
					...
					onHiding:function(){
						 self.FunnelChart.Palette = chagePalette;
					}
                });
				...
				p.option({
					...
					contentTemplate: function(contentElement) {
						...
						$('#ok-hide').on('click', function() {
                            ...
                            self.FunnelChart['Palette'] = newPalette;
...
//						}
						...
						$('#' + self.itemid + '_toggleLegend').dxButton({
                            text: '범례 표시',
							onClick: function() {
								self.FunnelChart.Legend.Visible = !self.FunnelChart.Legend.Visible;
								self.dxItem.option('legend.visible', self.FunnelChart.Legend.Visible);
							}
                        });
                        
						$.each($('.select-position'), function(index, position) {
							if (self.FunnelChart.Legend.Position === $(position).data('description')) {
						...
						$('.select-position').off('click').on('click', function(e) {
							...
							self.FunnelChart.Legend.Position = newDescription;
				...
				p.option({
					...
					contentTemplate: function(contentElement) {
						...
						$('#' + self.itemid + '_labelLocation').dxRadioGroup({
							width: '60px',
							dataSource: ['컬럼', '내부', '외부'],
							value: mapping[self.FunnelChart.LabelPosition],
							onValueChanged: function(e) {
								var position = mapping[e.value];
								self.FunnelChart.LabelPosition = mapping[position];
...
//                            var filter = $('#' + self.itemid + '_editFilter').dxFilterBuilder('instance').getFilterExpression();
			...
			case 'singleMasterFilter': {
				...
				setTimeout(function () {
					...
					if (self.IO.MasterFilterMode === 'Single') {
						...
						self.FunnelChart.InteractivityOptions.MasterFilterMode = 'Off';
					...
					// toggle on
						...
						self.FunnelChart.InteractivityOptions.MasterFilterMode = 'Single';
			...
			case 'multipleMasterFilter': {
				...
				setTimeout(function () {
					...
					if (self.IO.MasterFilterMode === 'Multiple') {
						...
						$('#' + self.trackingClearId).removeClass('invisible');
						self.FunnelChart.InteractivityOptions.MasterFilterMode = 'Multiple';
			...
			case 'ignoreMasterFilter': {
				...
				self.FunnelChart.InteractivityOptions.IgnoreMasterFilters = self.IO.IgnoreMasterFilters;
			...
			case 'crossFilter': {
				...
				self.FunnelChart.IsMasterFilterCrossDataSource = self.IsMasterFilterCrossDataSource;
				gProgressbar.show();
				setTimeout(function () {
					self.functionBinddata = true;
			...
			case 'drillDown': {
				...
				if ($('#drillDown').hasClass('on')) {
						...
						self.FunnelChart.InteractivityOptions.IsDrillDownEnabled = true;
...
WISE.libs.Dashboard.FunnelChartFieldManager = function() {
	var self = this;
