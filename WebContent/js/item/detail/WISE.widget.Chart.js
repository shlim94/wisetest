...
//	    }
		...
		var dxConfigs = {
			...
			onDrawn: function() {
				...
				if(WISE.Constants.editmode == 'viewer') {
					$('#' + self.ComponentName).css('margin-bottom', '-1.6%');
				}
			...
			case 'editTimeseries': {
				...
				p.option({
					...
					contentTemplate: function(contentElement) {
                        ...
                        '<div id="timeSeriesForm"></div>' +
                        '<p style="color: black; margin-top: 10px">분석 기간:</p>'+
                        '<div id="timeSeriesPeriod" style="margin-top: 10px"></div>' +
                        '<div id="colorContainer"></div></div>';
                        contentElement.append(html);
                        ...
                        colorContainer.dxScrollView({
							height: self.dxItem.option('series').length > 3? 200: 'auto',
                            width: '100%'
                        }).appendTo(contentElement);
						var html = 	'<div style="padding-bottom:20px;"></div>' +
									'<div class="modal-footer" style="padding-bottom:0px;">' +
						...
						var timeSeriesForm = $("#timeSeriesForm").dxForm({
							items: [
								{
									dataField: '배치 크기',
									editorType: 'dxNumberBox',
									editorOptions: {
										...
										showSpinButtons: true,
									    onKeyDown(e) {
									      var str = e.event.key || String.fromCharCode(e.event.which);
									      if (/^[.,e]$/.test(str)) {
						...
						var timeSeriesPreriod = $("#timeSeriesPeriod").dxRangeSelector({
							scale: {
								startValue: new Date(2011, 1, 1),
								endValue: new Date(2016, 1, 1),
								minorTick: {
									visible: false,
								},
							},
