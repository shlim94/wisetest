WISE.libs.Dashboard.item.ScatterPlotGenerator = function() {
	var self = this;
...
//				.toArray(_item.DataItems.Dimension) : [];
	...
	this.setScatterPlot = function(){
		this.fieldManager.init();
		gDashboard.itemGenerateManager.getherFields(this.fieldManager, self.type);
		this.ScatterPlot['ComponentName'] = this.ComponentName;
		this.ScatterPlot['Name'] = this.Name;
		this.ScatterPlot['DataSource'] = this.dataSourceId;
		
		this.ScatterPlot['DataItems'] = this.fieldManager.DataItems;
		this.ScatterPlot['Arguments'] = this.fieldManager.Arguments;
		this.ScatterPlot['Values'] = this.fieldManager.Values;
	
		this.measures = this.fieldManager.measures;
		this.dimensions = this.fieldManager.dimensions;
		this.ScatterPlot.HiddenMeasures = self.fieldManager.HiddenMeasures;
		
		this.meta = this.ScatterPlot;
		
		if (!(this.ScatterPlot['Palette'])) {
			this.ScatterPlot['Palette'] = userJsonObject.userPalette ? userJsonObject.userPalette : userJsonObject.defaultPalette;
		}
		// 20200901 ajkim 마스터 필터 추가 dogfoot
		if (this.ScatterPlot.InteractivityOptions) {
			if (!(this.ScatterPlot.InteractivityOptions.MasterFilterMode)) {
				this.ScatterPlot.InteractivityOptions.MasterFilterMode = 'Off';
			}
			if (!(this.ScatterPlot.InteractivityOptions.TargetDimensions)) {
				this.ScatterPlot.InteractivityOptions.TargetDimensions = 'Argument';
			}
			if (!(this.ScatterPlot.InteractivityOptions.IgnoreMasterFilters)) {
				this.ScatterPlot.InteractivityOptions.IgnoreMasterFilters = false;
			}
		} else {
			this.ScatterPlot.InteractivityOptions = {
		...
		if (!(this.ScatterPlot['Legend'])) {
			this.ScatterPlot['Legend'] = {
					Visible : true,
					Position : "RightOuter"
			}
		}
		
		if (!(this.ScatterPlot.AxisY)) {
			this.ScatterPlot.AxisY = {
				FormatType: 'Number',
				Unit: 'Ones',
				ShowZero: true,
				Visible: true,
		...
		if (!(this.ScatterPlot.AxisX)) {
			this.ScatterPlot.AxisX = {
				FormatType: 'Number',
				Unit: 'Ones',
				ShowZero: true,
				Visible: true,
		...
		if(!(this.ScatterPlot.Round)){
			this.ScatterPlot.Round = {
					Min: 5,
					Max: 20
			}
		}
		
		if(!this.ScatterPlot.LayoutOption){
			this.ScatterPlot.LayoutOption = {
					Legend : {
						family: '맑은 고딕',
						size: 12
					},
		...
		if(!this.ScatterPlot['ZoomAble']){
			this.ScatterPlot.ZoomAble = 'none'
		}
	};
	
	this.setScatterPlotforOpen = function(){
		if(typeof this.meta == 'undefined'){
			this.setScatterPlot();
		}
		else{
			this.ScatterPlot = this.meta;
		}
		if(typeof this.meta.DataSource == 'undefined'){
			this.meta['DataSource'] = 'dataSource1';
		}
		if(this.fieldManager != undefined){
			this.ScatterPlot['DataItems'] = this.meta['DataItems'] = this.fieldManager.DataItems;
			this.ScatterPlot['Arguments'] = this.meta['Arguments'] = this.fieldManager.Arguments;
			this.ScatterPlot['Values'] = this.meta['Values'] = this.fieldManager.Values;
			this.ScatterPlot.HiddenMeasures = self.fieldManager.HiddenMeasures;
			this.measures = this.fieldManager.measures;
			this.dimensions = this.fieldManager.dimensions;
		}
		...
		if (!(this.ScatterPlot['Palette'])) {
			...
			if(gDashboard != undefined){
				var ScatterPlotOption = typeof gDashboard.structure.MapOption.DASHBOARD_XML == 'undefined' ? '' : WISE.util.Object.toArray(gDashboard.structure.MapOption.DASHBOARD_XML.SCATTER_PLOT_DATA_ELEMENT);
				
				$.each(ScatterPlotOption,function(_i,_ScatterPlotOption){
					var CtrlNM;
					if (page[page.length - 1] === 'viewer.do'){
						CtrlNM = _ScatterPlotOption.CTRL_NM +"_"+WISE.Constants.pid;
					}else{
						CtrlNM = _ScatterPlotOption.CTRL_NM;
					}
					if(self.ScatterPlot.ComponentName == CtrlNM){
						self.ScatterPlot['Palette'] = _ScatterPlotOption.PALETTE_NM;
						return false;
					}
				})
				if(typeof this.meta.ColorSheme != 'undefined'){
					var colorList = WISE.util.Object.toArray(this.meta.ColorSheme.ColorSheme);
					this.ScatterPlot.Palette = new Array();
					var newPalette = [];
					$.each(colorList,function(_i,_list){
						self.ScatterPlot.Palette.push(gDashboard.itemColorManager.getHexColor(_list.Color)); 
	...
	this.setScatterPlotForViewer = function(){
		if(typeof this.meta == 'undefined'){
			this.setScatterPlot();
		}
		else{
...
//					}
	...
	this.bindData = function(_data) {
		...
		if(this.fieldManager !=null && gDashboard.isNewReport == true){ // 신규 생성
			...
			gDashboard.itemGenerateManager.itemCustomize(self,self.ScatterPlot);
			gDashboard.itemGenerateManager.generateItem(self, self.ScatterPlot);
		}else if(this.fieldManager !=null && gDashboard.isNewReport != true){
			gDashboard.itemGenerateManager.getherFields(this.fieldManager, self.type);
			this.setScatterPlotforOpen();
			gDashboard.itemGenerateManager.addParentItems(self);
			gDashboard.itemGenerateManager.itemCustomize(self,self.ScatterPlot);
			gDashboard.itemGenerateManager.generateItem(self, self.ScatterPlot);
		}
		//d3 뷰어모드 
		else if(self.meta && $.isEmptyObject(self.ScatterPlot)) {
			this.setScatterPlotForViewer();
			gDashboard.itemGenerateManager.itemCustomize(self,self.ScatterPlot);
			gDashboard.itemGenerateManager.generateItem(self, self.ScatterPlot);
...
//								self.dimensions.splice(_i,1);
		...
		self.fScatterPlot(_data, this.measures, this.dimensions, self.deleteDuplecateData(_data,measureKey));
...
//		gDashboard.itemGenerateManager.generateDxItem(self, _data);
	...
	this.resize = function() {
		if(typeof self.resizeData != 'undefined'){
    		...
    		WISE.loadedSourceCheck('d3');
			self.fScatterPlot(self.filteredData, self.measures, self.dimensions, self.resizeData);
...
//	this.resize = function() {
//		
//		self.fScatterPlot(self.filteredData, self.measures, self.dimensions, 
...
//		gProgressbar.hide();
//	};
	this.fScatterPlot = function(jsonData, measures, dimensions, dupleData) {
		if(gDashboard.reportType === "StaticAnalysis"){
			if(self.focusItemType === "logisticRegression"){
                self.fScatterPlotNormal(jsonData, measures, dimensions, dupleData)
			}else{
				self.fScatterPlotAys(jsonData, measures, dimensions, dupleData)
			}
	...
	this.fScatterPlotNormal = function(jsonData, measures, dimensions, dupleData) {
		...
		/*dogfoot 로지스틱 다중일때 시각화 숨김 shlim 20201111*/
		if(self.focusItemType && self.focusItemType === "logisticRegression"){
			if(dimensions.length > 2){
				return false;
			}
		}
		
		var wrapper = d3.select("#"+self.itemid)
		.append("svg").attr("id", self.itemid+"_wrapper")
		...
		var Ycount = 1;
		var scatterPlot = function(g, dupleData, svgWidth, svgHeight, option){
			...
			if(self.meta.Legend.Position === "LeftOuter" && self.meta.Legend.Visible){
				...
				margin.bottom = 120;
			}else if(self.meta.Legend.Visible && self.meta.Legend.Position.indexOf("Inner") == -1){
				margin.top = 100;
			}
			...
			.attr("x", option.left).attr("y", option.top)
			.append("g")
			.attr("transform",
			...
			.attr("x", 0)
			.attr("y", -20 )
			.text(typeof self.meta.AxisY.Title != 'undefined'? self.meta.AxisY.Title : self.dimensions[Ycount].caption )
			...
			var tooltip;
			if($("#"+self.itemid + " .scatter-plot-tooltip")){
				tooltip = d3.select("#"+self.itemid + " .scatter-plot-tooltip div");
			}else{
				tooltip = d3.select("#"+self.itemid)
				.append("div")
				.style("opacity", 0)
			...
			var showTooltip = function(d) {
				...
				if(gDashboard.reportType == 'StaticAnalysis'){
					...
					tooltipHtml += ('<br>' + d.dimensionNm + ': ' + d[self.dimensions[1].caption]);
				}else{
					tooltipHtml += ('<br>' + self.dimensions[0].caption + ': ' + d[self.dimensions[0].caption]);
			...
			var highlight = function(d){
				...
				if(gDashboard.reportType === "StaticAnalysis"){
					d3.selectAll('._path' + self.itemid).style("opacity", 0.05)
					d3.selectAll('#' + self.itemid + d).style("opacity", 0.8)
				}
			}
			...
			var noHighlight = function(d){
				...
				if(gDashboard.reportType === "StaticAnalysis"){
					d3.selectAll('._path' + self.itemid).style("opacity", 0.8)
				}
			}
			...
			.attr("class", function(d) {
				if(!d["realdata"]){
				    return 'bubbles _' + d['dimension'].replace(/(\s*)/g, "") + " _pathdata"; 	
				}else{
					return 'bubbles _' + d['dimension'].replace(/(\s*)/g, "");
				}
			...
			.style("fill", function (d) {
				if(!d["realdata"]){
				    return myColor( d['dimension'] || d['dimensionNm']); 	
				}else{
					return myColor( d['dimension']);
				}
			} )
			.style("opacity", 0.8)
			...
			if(self.focusItemType && self.focusItemType === "logisticRegression"){
				...
				svg.append("path")
				  ...
				  .attr("stroke", function (d) {
						return myColor( d['dimensionNm']); 	
					})
				  .attr("stroke-width", 1.5)
				  .attr("d", 
				  	    d3.line().curve(d3.curveMonotoneX)
					                    .x(function(d) { return x(d[self.dimensions[0].name]) })
					                    .y(function(d) { return y(d[self.dimensions[1].name]) })
				   
				  )
				  d3.selectAll('#' + self.itemid + ' ._pathdata').attr("display",'none');
			}else{
				var aysArrY = [],
				aysArrX = [];
				/*dogfoot 통계 산점도 선 색상 추가 shlim 20201109*/     

				$.each(self.dimensions,function(_j,_dim){
					if(_dim.caption == data[0].dimensionNm){
                        if(_j != 0){
						var arrinstY=[];
						var arrinstX=[];
						$.each(self.resizeData,function(_i,_data){
							if(_dim.caption == _data.dimensionNm){
								arrinstX.push(_data[self.dimensions[0].caption])	
								arrinstY.push(_data[self.dimensions[1].caption])
							}

						})
						aysArrX.push(arrinstX);
						aysArrY.push(arrinstY);	
					    }
					}
					
				})
				for(var i = 0; i<aysArrX.length; i++){
					regression=leastSquaresequation(aysArrX[i],aysArrY[i]);
					...
					.x(function(d) { return x(d[self.dimensions[0].caption]); })
					.y(function(d) { return y(regression(d[self.dimensions[0].caption])); });
					...
					.data([data]) // [] 안에 업데이트
					.attr("fill", "none")
					.attr("stroke",  function (d) { return myColor("선 색상"); })			/*dogfoot 통계 산점도 선 색상 추가 shlim 20201109*/     
					.attr("stroke-width", 2)
					.attr("d", line)
					.attr("id",self.itemid + self.dimensions[i+1].caption)
					.attr("class","_path"+self.itemid);
					}	
				}
			}
		}
		...
		if(gDashboard.reportType !== 'StaticAnalysis'){
			scatterPlot(wrapper, dupleData, $("#"+self.itemid).width(), $("#"+self.itemid).height(), {top : 0, left: 0});
		}else{
			var panelData = self.__calcplotPanelSize(self.paletteData.length - 1);
			$.each(self.paletteData.slice(0, self.paletteData.length - 1), function(i, d){
				var x = i % panelData.r * panelData.w;
				var y = i == 0? 0 : Math.floor(i / panelData.r ) * panelData.h;
				
				var tempData = dupleData.filter(function(x) {
					return x.dimension == d;
				})

				scatterPlot(wrapper, tempData, panelData.w, panelData.h, {top : y,  left: x});
				Ycount++
			});
		}
	};
	
	this.fScatterPlotAys = function(jsonData, measures, dimensions, dupleData) {
		...
		var scatterPlot = function(g, dupleData, svgWidth, svgHeight, option){
			...
			// Add X axis
			var data = dupleData;
			var arrX = data.map(function(d){return d[self.dimensions[1].caption]});
			var maxX = d3.max(arrX);
			var minX = d3.min(arrX);
			...
			.attr("x", width)
			.attr("y", height+50 )
			.text(typeof self.meta.AxisX.Title === 'undefined'? self.dimensions[Ycount].caption : self.meta.AxisX.Title)
			.attr("font-size", self.meta.LayoutOption.AxisX.size * 1 + 2)
			.attr("font-family", self.meta.LayoutOption.AxisX.family)
			.style("fill", self.meta.LayoutOption.AxisX.color)

			var arrY = data.map(function(d){return d[self.dimensions[0].caption]});
			var maxY = d3.max(arrY);
			var minY = d3.min(arrY);
			minY -= (maxY - minY)/10;
			...
			.attr("x", 0)
			.attr("y", -20 )
			.text(typeof self.meta.AxisY.Title != 'undefined'? self.meta.AxisY.Title : self.dimensions[0].caption )
			.attr("text-anchor", "start")
			.attr("font-size", self.meta.LayoutOption.AxisY.size * 1 + 2)
			.attr("font-family", self.meta.LayoutOption.AxisY.family)
			.style("fill", self.meta.LayoutOption.AxisY.color)
			...
			.attr("cx", function (d) { return typeof arrX[0] === 'string' ? x(arrX.indexOf(d[self.dimensions[1].caption])) : x(d[self.dimensions[1].caption]); } )
			.attr("cy", function (d) { return typeof arrY[0] === 'string' ? y(arrY.indexOf(d[self.dimensions[0].caption])) : y(d[self.dimensions[0].caption]); } )
			.attr("r", function (d) { return z(self.measures[0]? d[self.measures[0].captionBySummaryType] : '')} )
			...
			if(self.focusItemType && self.focusItemType === "logisticRegression"){
				...
				$.each(self.dimensions,function(_j,_dim){
					if(_dim.caption == data[0].dimensionNm){
						...
						$.each(self.resizeData,function(_i,_data){
							if(_dim.caption == _data.dimensionNm){
								arrinstX.push(_data[self.dimensions[1].caption])	
								arrinstY.push(_data[self.dimensions[0].caption])
							}
				...
				for(var i = 0; i<aysArrX.length; i++){
					...
					var line = d3.svg.line()
					.x(function(d) { return x(d[self.dimensions[1].caption]); })
					.y(function(d) { return y(regression(d[self.dimensions[1].caption])); });
	...
	this.__calcplotPanelSize = function(_plotCount) {
		var ww = $('#'+this.itemid).width();
		var hh = $('#'+this.itemid).height();
		var itemPanelMinSize = 700;
        var itemPanelHeightSize, itemPanelWidthSize;
        var itemPanelRow, itemPanelCol;
        ...
        if (ww < itemPanelMinSize) {
            ...
            itemPanelRow = _plotCount < (_i+1) ? _plotCount : (_i+1);
            
            itemPanelWidthSize = parseInt(ww / itemPanelRow, 10);
            itemPanelWidthSize = itemPanelWidthSize - 15; // 15: scroll width
        }
        
        itemPanelCol = Math.ceil(_plotCount / itemPanelRow);
        
    	itemPanelHeightSize = hh / itemPanelCol;
        
//        if (itemPanelHeightSize > itemPanelMinSize) {
//        	itemPanelHeightSize = hh - 4;  // padding-right, left : 2px
//        }
//        
        return {w: itemPanelWidthSize, h: itemPanelHeightSize, c: itemPanelCol, r: itemPanelRow};
	};
};


WISE.libs.Dashboard.ScatterPlotFieldManager = function() {
	var self = this;
